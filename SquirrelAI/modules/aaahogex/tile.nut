...
class HgTile {
	...
	function IsBuildable() {
		return SquirrelAI.IsBuildable(tile);
	}
...
//		return HgTile.XY(this.X() - hgTile.X(), this.Y() - hgTile.Y());
	...
	static function LevelBound( t1, t2, level, checkWaterLeak = true) {
		...
		foreach(c in HgTile(t1).GetConnectionCorners(HgTile(t2))) {
			...
			if(currentLevel != level && SquirrelAI.Get().IsAvoidRemovingWater() && HgTile.IsAroundRiverCorner(HgTile.GetCornerTile(t1,c))) {
				return false;
			}
	...
	static function IsLandConnectedForRail(p1, p2) {
		if(SquirrelAI.Get().CanRemoveWater()) {
			return true;
		}
	...
	static function IsLandConnected(start, end, allowedSeaLength) {
		...
		local p1 = end; //pathfinder?????????start < end ? start : end;
...
//			HgLog.Info("test BuildRailDepot:"+HgTile(depotTile)+","+HgTile(tile)+" "+AIError.GetLastErrorString());
	...
	static function BuildRoadDepot(depotTile,front,demolished=false) {
		...
		if(AIRoad.BuildRoadDepot(depotTile, front)) {
			...
			if( RoadRoute.IsTramCurrent() ) {
				AITile.DemolishTile(depotTile); // ???????????depot??????????
			}
			if(!AIRoad.BuildRoadDepot( depotTile, front )) {
				return false;
			}
			return true;
		} else if(!demolished && SquirrelAI.Get().IsRich() && BuildUtils.CanTryToDemolish(depotTile)) {
	...
	static function BuildWaterDepot(depotTile,front,force=false) {
		...
		if(AIMarine.IsWaterDepotTile(depotTile) 
				&& AICompany.IsMine(AITile.GetOwner(depotTile))
				/*&& AIMarine.AreWaterTilesConnected(front, depotTile) ????????????????*/) {
			return true; // ???
		}
...
class Rectangle {
	...
	static function Center(centerHgTile, radius) {
		...
		local right = min(AIMap.GetMapSizeX() - 2, x + radius + 1); // TODO:??????????????????????????????
	...
	function Width() {
		...
		return rightbottom.tile % w - lefttop.tile % w;  // rightbottom????????????????????????  rightbottom.X() - lefttop.X();
	}
	...
	function GetRightBottomTile() {
		return rightbottom.tile - AIMap.GetMapSizeX() - 1; // rectangle?????map????1?????????? HgTile.XY( rightbottom.X()-1, rightbottom.Y()-1 );
	}
	...
	function IsBuildable() {
		return SquirrelAI.IsBuildableRectangle(lefttop.tile, Width(), Height());
	}
...
class TileListUtils {
	...
	static function LevelAverage(tileList, track, isTestMode = false, average = null, force = false) {
		local landfill = AICompany.GetBankBalance(AICompany.COMPANY_SELF) > SquirrelAI.Get().GetInflatedMoney(500000);
		if(average == null) {
		...
		foreach(tile,level in tileList) {
			if(!isTestMode) {
				level = AITile.GetCornerHeight(tile, AITile.CORNER_N); // ??????????????????????
			}
			if(level < average) {
				if(raiseTileMap == null || raiseTileMap.rawin(tile)) {
					foreach(d in around) {
						local t = tile + d;
						if(AITile.IsWaterTile(t) /*level <= 0 ??????????? */) {
							if(!landfill) {
								return false;
							}
			...
			foreach(tile,lowerTiles in needsChecks) {
				if(lowerTiles.len() >= 3) {
					...
					if(AIRoad.IsRoadTile(tile)) {
						...
						if(AIRoad.AreRoadTilesConnected(tile, ngTile)) { // ???????????????????????
							return false;
						}
					}
	...
	static function CalculateRaiseTileMap(tiles, track) {
		...
		foreach(_,xy in tilesXY) {
			if(TileListUtils.GetBothCount(xy,tilesXY) == 1 && TileListUtils.GetSequenceCount(xy,tilesXY) >= 2) {
			} else {

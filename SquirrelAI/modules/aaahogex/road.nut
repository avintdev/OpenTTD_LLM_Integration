...
class RoadRoute extends CommonRoute {
	...
	function GetMaxTotalVehicles() {
		return SquirrelAI.Get().maxRoadVehicle;
	}

	function GetThresholdVehicleNumRateForNewRoute() {
		return 0.8; //??????????????????? CommonRoute.IsSupportModeVt(AIVehicle.VT_ROAD) ? 0.80 : 0.95;
	}
	...
	function GetRouteInfrastractureCost() {
		if(!SquirrelAI.Get().IsInfrastructureMaintenance()) return 0;
		local usedMap = RoadRoute.GetUsedMap(IsTram());
		local pre = null;
		local roadTypeAcc = {};
		local isTram = IsTram();
	...
	function SetPath(path) {
		local needDepot = GetDistance() >= 150 && SquirrelAI.Get().IsEnableVehicleBreakdowns();
		local execMode = AIExecMode();
		local count = 0;
		local isTram = IsTram();
	...
	function OnVehicleLost(vehicle) {
		...
		local execMode = AIExecMode();
		//if(SquirrelAI.Get().IsInfrastructureMaintenance()) return; // ????????????
		if(isRemoved) return; // ?????????
		
		/*if(lastRebuildDate != null && lastRebuildDate + 7 > AIDate.GetCurrentDate()) ????????????? {
		...
		//if(!roadBuilder.BuildPath(destHgStation.GetEntrances(), srcHgStation.GetEntrances(), true)) {
			...
			//TODO: ??????????depot??order????????????order??????????Route.Remove?????
			local isTram = IsTram();
			foreach(tile in usedTiles) {
	...
	static function _DemolishLines(lines,isTram) {
		...
		foreach(removeTiles in lines) {
			...
			foreach(tile in removeTiles) {
				if(vehicleLocations.rawin(tile)) { // TODO ?????????????????
...
class RoadRouteBuilder extends CommonRouteBuilder {
	...
	function CreateStationFactory(target,engineSet) { 
		if(RoadRoute.IsTramCurrent()) {
			return RoadStationFactory(cargo,false,engineSet); // Tram????piecestation????????U???????
		} else {
			// TODO: ????????
			if(SquirrelAI.Get().roiBase && target instanceof TownCargo /*&& AITown.GetPopulation(target.town) < 1300*/ && !SquirrelAI.Get().IsDistantJoinStations()) {
	...
	function GetSuitableRoadType(engineSet) {
		...
		list.Valuate( AIRoad.GetBuildCost, AIRoad.BT_ROAD ); // TODO: ??????????????????????(????roadtype?????)
...
class RoadBuilder {	
	...
	constructor(engine=null, cargo=null) {
		...
		this.retryCount = 0;
		this.ignoreTiles = [];
		this.pathFindLimit = SquirrelAI.Get().IsInfrastructureMaintenance() ? 300 : (SquirrelAI.Get().roiBase ? 100 : 50);
		this.retry = false;
		this.debug = false;
	}
	...
	function FindPath(starts, goals, suppressInterval, isRetry=false) {
		...
		if(SquirrelAI.Get().roiBase || SquirrelAI.Get().IsInfrastructureMaintenance()) {
		...
		/* if(IsConsiderSlope()) { ???????
			...
			pathFindLimit = SquirrelAI.Get().roiBase ? 200 : 100;
		}*/
		local distance = AIMap.DistanceManhattan(starts[0],goals[0]);
		if(SquirrelAI.Get().IsInfrastructureMaintenance() || SquirrelAI.Get().IsPreferReusingExistingRoads()) {
//			pathfinder._cost_no_existing_road = 40; //distance < 150 ? 200 : 40 // ??????200??????
			pathfinder._cost_tile = 50;
			pathfinder._cost_no_existing_road = 140;
			if((SquirrelAI.Get().IsInflation() || SquirrelAI.Get().IsPreferReusingExistingRoads()) && !isRetry) {
				retry = true;
				pathfinder._estimate_rate = 1; //?????????????
				pathFindLimit = 300; // ??????????????????????????max(300, distance * distance / 30);
			}
		} else {
			pathfinder._cost_tile = 50;
			pathfinder._cost_no_existing_road = 140;
...
//			pathfinder._cost_no_existing_road = 40;
	...
	function RetryBuildRoad(curPath, goals) {
		...
		roadBuilder.ignoreTiles.push(curPath.GetTile());// ???????????????????????????? => ?????????PathFinder?????????????????????
...
class TownBus {
	...
	static function CanUse(cargo = null) {
		if(SquirrelAI.Get().IsDisableRoad()) { // Tram?townbus?????????
			return false;
		}
		if(SquirrelAI.Get().IsInflation()) {
			return false; // ??????townbus????????????
		}
		if(cargo==null) {
			cargo = SquirrelAI.GetPassengerCargo();
		}
	...
	static function IsReadyEconomy() {
		return SquirrelAI.Get().IsRich() || (SquirrelAI.Get().GetUsableMoney() >= SquirrelAI.Get().GetInflatedMoney(200000) && SquirrelAI.Get().HasIncome(50000));
	}
	...
	function CheckRetry(authorityTown, ignoreTileList, cargo) {
		if(AIDate.GetCurrentDate() > date + 365 && (stations.len() < 2 || depot == null)) {
			...
			return TownBus.CheckTown(authorityTown, ignoreTileList, cargo); // TODO: TownBus.instances?????
		} else {
			return this;
		}
	}
	...
	function BuildBus() {
		...
		SquirrelAI.WaitForPrice(AIEngine.GetPrice(busEngine));
	...
	function GetOrderStations(bus) {
		...
		for(local index = SquirrelAI.Get().IsEnableVehicleBreakdowns() ? 1 : 0; index < orderCount; index++) {
...
/*		vehicleList.Valuate( AIVehicle.IsStoppedInDepot );
	...
	function BuildBusStops() {
		local currentRoadType = AIRoad.GetCurrentRoadType();
		AIRoad.SetCurrentRoadType(GetRoadType()); // TODO tram??
	...
	function BuildBusDepot() {
		if(cargo != SquirrelAI.GetPassengerCargo()) {
			local key = town+":"+SquirrelAI.GetPassengerCargo();
			if(TownBus.townMap.rawin(key)) {
	...
	function CheckInterval() {
		...
		if(!isTransfer && AIBase.RandRange(100) < 5 && SquirrelAI.Get().IsRich()) {
			CheckRenewal();
		}
	}
	...
	function RemoveBus() {
		...
		if(removeBus != null) {
			HgLog.Warning("RemoveBus failed.removeBus != null "+this); // ??????????
			return;
		}
	...
	function CheckRenewal() {
		...
		if(engine != AIVehicle.GetEngineType(bus) || (SquirrelAI.Get().IsEnableVehicleBreakdowns() && AIVehicle.GetAgeLeft(bus) <= 600)) {
			RemoveBus();
			BuildBus(); //too many vehicles???townbus??????
		}
	}
	...
	function CreateTransferRoutes(route, placeStation) {
		SquirrelAI.Get().supressInterval = true; // TownBus.CheckInterval???????
		local currentRoadType = AIRoad.GetCurrentRoadType();
		...
		AIRoad.SetCurrentRoadType(currentRoadType);
		SquirrelAI.Get().supressInterval = false;
	}
	...
	function _CreateTransferRoutes(route, placeStation) {
		...
		if(SquirrelAI.Get().CanExtendCoverageAreaInTowns() && CargoUtils.IsPaxOrMail(cargo)) {
		...
		for(local i=0; i<stations.len(); i++) {
			...
			if(toHgStation == null) {
				...
				if(toHgStation == null && !placeStationCanBeTransfer/*road station????piace station???????*/) {
			...
			local busEngine = TownBus.GetStandardBusEngine(cargo); /*Road type??????*/
			if(busEngine == null) {
				HgLog.Warning("Cannot detect road type(Not found bus engine)"+this);
				return;
			}
			if(CanBuildNewBusStop()) { // ???????????5??????????????????
				// TODO: join??????join??????
				local busStop = FindNewBusStop(SquirrelAI.GetPassengerCargo() == cargo ? 60 : 30, placeStation);
				if(busStop != null) {
	...
	function CreateRoadBuilder(busEngine) {
		...
		//result.maxCost = 5000; // 50tile?? ??????????limit??????????????
	...
	function CanBuildNewBusStop() {
		...
		if(SquirrelAI.Get().CanExtendCoverageAreaInTowns()) {
			poplulationMultiplier = SquirrelAI.Get().maxStationSpread.tofloat() / 6;
	...
	//static
	function GetRoadType() {
		return SquirrelAI.Get().townRoadType;
		//return AIRoadTypeList(AIRoad.ROADTRAMTYPES_ROAD).Begin();	
	}
	...
	function CanUseTownBus() {
		return RoadRoute.EstimateEngineSet(RoadRoute, SquirrelAI.GetPassengerCargo(), 10,  50, true, null, true ) != null;
	}
	...
	function FindNewBusStop(acceptanceThreshold,placeStation) {
		...
		foreach(station in stations) {
			if(AIStation.HasCargoRating(station.stationId,cargo) || station == placeStation/*?????Rating??*/) {

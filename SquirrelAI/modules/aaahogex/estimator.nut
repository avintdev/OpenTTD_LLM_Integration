...
Estimation <- {
	function GetBaseCost() {
		...
		} else if(SquirrelAI.Get().buildingTimeBase) {
			return buildingTime;
		} else {
	...
	function GetMaxRouteCapacity(cargo) {
		...
		if(length < 128 && platformLength != null && length/16 < platformLength-1) { // length < 8 tile??
	...
	function CalculateIncome() {
		...
		income = SquirrelAI.Get().futureIncomeRate * income / 100 - runningCost;
	...
	function AppendSubCargoIncome(subCargoEstimate, supportBuildingTime) {
		...
		//incomePerOneTime += subCargoEstimate.incomePerOneTime; // ?????????????????
	...
	function GetValue() {
		...
		local usableMoney = SquirrelAI.Get().GetUsableMoney();
		local yearlyIncome = SquirrelAI.Get().GetQuarterlyIncome(4);
		if(isBidirectional) {
		...
		return SquirrelAI.Get().GetValue(roi,incomePerBuildingTime,incomePerVehicle);
	}
	...
	function EstimateAdditional( dest, src, infrastractureTypes, destinationRoute = null, sourceRoute = null, subCargo = false, notUseSupportRoutes = false, isMinSupport = false ) {
		...
		if(destinationRoute != null) {
			...
			local oldTotalDistance = totalDistance; // cargo??
			totalDistance = AIMap.DistanceManhattan( dest.GetLocation(), src.GetLocation() ); // ?????cargo??
			...
			if(dest instanceof Place) {
				...
				local additionalRouteIncome = dest.GetAdditionalRouteIncome(cargo); // ??cargo?????
				if(destRouteCargoIncome > 0 || additionalRouteIncome > 0) {
					this.destRouteCargoIncome = destRouteCargoIncome; // ???????????? * min(100 / (dest.GetUsedOtherCompanyEstimation()+1),70) / 100; // TODO: ????
...
//					Estimate();
		...
		if( !notUseSupportRoutes && SquirrelAI.Get().buildingTimeBase ) {
		...
		if(!subCargo && !isMinSupport && vehicleType == AIVehicle.VT_RAIL) { // ?????cargo???????
	...
	function Estimate() {
		...
		if(isBidirectional) {
			if(vehicleType == AIVehicle.VT_AIR) {
				incomePerOneTimeReturn = cargoIncome * capacity; // dest???????
			} else {
		...
		if( vehicleType == AIVehicle.VT_AIR && cargo == SquirrelAI.GetPassengerCargo() && isBidirectional) {
			// TODO: ????????????
			local mail = SquirrelAI.GetMailCargo();
			if(mail != null) {
...
//								HgLog.Info("p:"+production+" stationRate:"+stationRate+" days:"+days+" c:"+capacity+" v:"+vehiclesPerRoute+" cargo:"+AICargo.GetName(cargo));
		...
		local interval = (cruiseDays * 2 + loadingTime)  / vehiclesPerRoute; // ??????isSingle??????????? //
		...
		foreach(cargo,capacity in cargoCapacity) {
			...
			incomePerOneTime += incomePerOneTimeReturn;
			cargoIncomes.rawset( cargo, cargoIncome + destRouteCargoIncome ); // cascade??????????????
	...
	function GetBuildingTime() {
		...
		return (result + Estimator.buildingTimePerVehicle * vehiclesPerRoute);// / 2; // Rail???????????????
	}
...
class Estimator {
	...
	function GetBuildingTime(distance, infrastracture) {
		// ???????????????rail?????????????
		switch(GetVehicleType()) {	//return distance / 2 + 100; // TODO expectedproduction???????????????
			case AIVehicle.VT_WATER:
		//		return (pow(distance / 20,2) + 150).tointeger(); //TODO: ?????????
		//		return distance * 2 + 1800; //TODO: ?????????
	...
	function GetMaxBuildingCost(infrastractureMaintenanceCost) {
		local incomeOneYear = max(0, SquirrelAI.Get().GetQuarterlyIncome(4) - infrastractureMaintenanceCost);
		return SquirrelAI.Get().GetUsableMoney() + incomeOneYear * 5;
	}
	...
	function GetBuildingCost(infrastractureType, distance, cargo) {
		switch(GetVehicleType()) {
			case AIVehicle.VT_RAIL:
				local demolishFarm = SquirrelAI.GetInflatedMoney(540) * 40 / 100;
				...
				return cost;
		//		return distance * railCost/*SquirrelAI.Get().GetInflatedMoney(720)*/ +SquirrelAI.Get().GetInflatedMoney( CargoUtils.IsPaxOrMail(cargo) ? 30000 : 20000);
		...
		/*			if(SquirrelAI.Get().roiBase) {
				...
				cost = SquirrelAI.Get().GetInflatedMoney(cost);
			...
			case AIVehicle.VT_ROAD:
				...
				cost += SquirrelAI.GetInflatedMoney(4000) + Route.GetPaxMailTransferBuildingCost(cargo);
		//		cost += SquirrelAI.GetInflatedMoney(CargoUtils.IsPaxOrMail(cargo) ? (!SquirrelAI.Get().IsDistantJoinStations() ? 25000 : 3500) : 2000);
				 return cost;
				 
			case AIVehicle.VT_AIR:
				local airportTraints = Air.Get().GetAiportTraits(infrastractureType);
				return SquirrelAI.Get().GetInflatedMoney(airportTraints.cost) * 2 * 2 /*??????*/ + Route.GetPaxMailTransferBuildingCost(cargo);
		}
...
class CommonEstimator extends Estimator {
	...
	function GetEngineSetsVt() {
		...
		local useReliability = SquirrelAI.Get().IsEnableVehicleBreakdowns();
		local breakdownDifficulty = SquirrelAI.Get().GetVehicleBreakdownDifficulty();
		...
		if(vehicleType == AIVehicle.VT_ROAD) {
			if(isRouteInstance) {
				...
				if(isTownBus || SquirrelAI.Get().IsDisableTrams()) {
					engineList.Valuate(AIEngine.HasPowerOnRoad, TownBus.GetRoadType());
					engineList.KeepValue(1);
				}
				if(SquirrelAI.Get().IsDisableRoad()) {
		...
		if(isRouteInstance) {
			ignoreIncome = self.IsSupport(); // ??????????????????? TODO: ??????????????????????
		}
		/*
		...
		local isBuildingEstimate = !isTownBus && !isRouteInstance /*??????*/
			...
			foreach(engineInfrastractureType in self.GetInfrastractureTypes(e)) {
				...
				if(!infrastractureEstimations.rawin(engineInfrastractureType)) {
					infrastracture = {
						...
						buildingCost = isTownBus/*???????????????*/ ? 0 : GetBuildingCost( engineInfrastractureType, distance, cargo )
					}
				...
				/*
				if(vehicleType == AIVehicle.VT_ROAD && SquirrelAI.Get().roadTrafficRate != null) { ???????????????????
					cruiseSpeed = (cruiseSpeed * SquirrelAI.Get().roadTrafficRate).tointeger();
				}*/
				...
				if(self.IsSupportModeVt(vehicleType) && self.GetMaxTotalVehicles() / 2 < AIGroup.GetNumVehicles( AIGroup.GROUP_ALL, vehicleType)) {
					maxVehicles = min(maxVehicles, 10); // ROAD?????????????????????????
				}*/
				...
				local loadingSpeed; // TODO: capacity/cargo/vehicleType???????
				switch(vehicleType) {
					case AIVehicle.VT_ROAD:
						loadingSpeed = 18; //?????????? 37;
				...
				*/
				...
				if(cargo == SquirrelAI.GetMailCargo() && vehicleType == AIVehicle.VT_AIR) {
					loadingSpeed /= 2; //???????????????????????
				}
				...
				if(vehicleType == AIVehicle.VT_ROAD) cruiseDays += 4; // ??????
				days = (cruiseDays + loadingTime) * 2;
				days = max(days,1);
				
				//ocal useTownBus = !isTownBus && TownBus.CanUse(cargo) && !SquirrelAI.Get().IsDistantJoinStations();
				...
				/*
				if(vehicleType == AIVehicle.VT_AIR && cargo==SquirrelAI.Get().GetPassengerCargo()) {
...
class TrainEstimator extends Estimator {
	...
	function CalculateSubCargoNumWagons(wagonEngineInfos, numMainWagon, totalNumWagon, locoCapacity) {
		...
		local production = productions[0] == 0 ? 50 : productions[0]; //0???????
		while(true) {
			local nums = [n0];
			local total = n0;
	...
	function GetWagonEngineInfo(cargo, engine) {
		...
		result.isFollowerForceWagon <- result.isMultipleUnit && !AIEngine.GetName(engine).find("Unpowered"); // ?????? TODO ????????????
		return result;
	}
	...
	function GetWagonSelectors(cargo,railSpeed) {
		...
		result.push(function(engine):(cargo,railSpeed) {
			...
			local cost = (AIEngine.GetPrice(engine) + AIEngine.GetRunningCost(engine) * 3) / SquirrelAI.GetInflationRate();
	...
	function _GetWagonEngines(cargo, railType) {
		...
		foreach(engine,_ in wagonEngines) {
			TrainInfoDictionary.Get().GetTrainInfo(engine);	 // Valuate???BuildTrain?????????
		}
	...
	function GetEngineSets() {
		...
		if(maxBuildingCost>0 && firstRoute) usableMoney -= SquirrelAI.GetInflatedMoney(20000); // ???????????????
		...
		foreach(wagonEngine,_ in GetWagonEngines(cargo, railType)) {
			if(limitWagonEngines != null && countWagonEngines >= limitWagonEngines) {
				...
				if(countWagonEngines >= limitWagonEngines+1) { // ??????wagon????????????????????????
			...
			if(checkRailType || limitTrainEngines != null) {
				if(checkRailType || limitTrainEngines == 1) {
					local money = SquirrelAI.Get().GetUsableMoney(); //max(200000,AICompany.GetBankBalance(AICompany.COMPANY_SELF));
			...
			foreach(trainEngine,v in trainEngines) {
				...
				local loadingTime = 4 * (isBidirectional ? 2 : 1); //???????????????????CargoUtils.IsPaxOrMail(cargo) ? 4 : 2;
				...
				for( ;;) {
					...
					usedPlatformLength = min(SquirrelAI.Get().maxStationSpread, usedPlatformLength);
					...
					if(firstRoute && price * 3 / 2 > usableMoney) {
						//HgLog.Warning("firstRoute && price * 3 / 2 > SquirrelAI.GetUsableMoney()");
					...
					local requestSpeed = SquirrelAI.Get().roiBase ? 10 : max(10, min(40, maxSpeed / 3));  // min(40, max(10, maxSpeed / 10));
...
//					local startDays = 8.0 / (trainPlan.GetPower().tofloat() / trainPlan.GetWeight().tofloat()) * cruiseSpeed / 100;
					...
					local stationInOutTime = 
						((VehicleUtils.GetDays( max(0,inOutDistance-startDistance),cruiseSpeed )  // ????????????
						+ VehicleUtils.GetDays( min(inOutDistance,startDistance),cruiseSpeed/2 )) // ???????????????
						+ 2) * (isRoRo ? 7 : 10) / 7;
					...
					local maxVehicles = max(1, days / max(stationLimitTime,stationInOutTime) + (isRoRo ? 2 : 0)); //+2??????
					...
					foreach(isSingle in [true,false]) {
						...
						if(SquirrelAI.Get().ecs && isSingle) { // ECS???????????????????????????
							continue;
						}
...
class TrainPlan {
	...
	function IncreaseNumWagon() {
		...
		if(wagonInfos.len() == 1) { // TODO wagonInfos.len()==0???
		...
		/*
			...
			foreach(wagonInfo in wagonInfos) {
				...
				if(maxWagonInfo == null || cp > maxCp) {
					if(wagonInfo.numWagon >= 1) { // loco?capacity?????0?????
	...
	function CalcuateTractiveEffortAndPower() {
		...
		foreach(wagonInfo in wagonInfos) {
			if(wagonInfo.isFollowerForceWagon) { // DMU / EMU???????
...
class TrainInfoDictionary {
	...
	function GetTrainInfo(engine) {
		...
		if(SquirrelAI.Get().maxTrains <= AIGroup.GetNumVehicles( AIGroup.GROUP_ALL, AIVehicle.VT_RAIL)) {
			return null;
		}
	...
	function CreateConnectedWagonInfo(depot, trainEngine, wagonEngine, cargo) {
		local exec = AIExecMode();
		if(AIEngine.GetPrice(trainEngine) + AIEngine.GetPrice(wagonEngine) > SquirrelAI.GetUsableMoney()) {
		...
		SquirrelAI.WaitForPrice(AIEngine.GetPrice(trainEngine));
		...
		SquirrelAI.WaitForPrice(AIEngine.GetPrice(engine));
	...
	function CreateTrainInfo(depot, engine) {
		...
		foreach(cargo, v in AICargoList()) {
			if(AIEngine.CanRefitCargo (engine, cargo)) {
				...
				/*
				if(price > SquirrelAI.Get().GetInflatedMoney(1000)) {
		...
		local isMultipleUnit = engineName.find("DMU") != null
				...
				|| AIEngine.GetWeight(engine) == 0; // TODO: ????????????

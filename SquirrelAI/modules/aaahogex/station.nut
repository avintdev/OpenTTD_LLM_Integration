...
class StationGroup {
	...
	constructor() {
		...
		producingCargoCache = ExpirationTable(3*365); // producing?????????
	...
	function GetStationCandidatesInSpread(stationFactory,checkedTile=null) {
		...
		local maxStationSpread = SquirrelAI.Get().maxStationSpread;
		...
		local result = null;
		if(SquirrelAI.Get().IsDistantJoinStations() == false) {
			result = [];
	...
	function GetLastMonthProduction( cargo )  {
		...
		foreach(transferRoute in GetUsingRoutesAsDest()) {
			if(transferRoute.IsTownTransferRoute()) { // TownCargo.GetExpectedProduction?????
				continue;
			}
	...
	function _GetExpectedProduction( cargo, vehicleType, isMine = false, callers = null, isFuture = false)  {
		local result = 0;
		foreach(place in GetProducingPlaces(cargo)) {
			/* local placeStationGroups = place.GetStationGroups(); ??????????
		...
		foreach(srcRoute in GetUsingRoutesAsDest()) {
			if(srcRoute.IsTownTransferRoute()) { // TownCargo.GetExpectedProduction?????
				continue;
			}
			//HgLog.Info("GetUsingRoutesAsDest:"+route+" "+this+" "+callers.rawin(route)+" "+route.HasCargo(cargo));
			if(callers == null) {
	...
	function GetCoasts(cargo=null) {
		...
		local tileList;
		if(SquirrelAI.Get().IsDistantJoinStations()) {
			tileList = GetBuildablePlatformRectangle(SquirrelAI.Get().maxStationSpread - 1).GetTileList();
		} else {
	...
	function GetBuildablePlatformRectangle(stationSpread = null, maxRadius = null) {
		if(stationSpread == null) {
			stationSpread = SquirrelAI.Get().maxStationSpread;
		}
		local r1 = null;
		foreach(hgStation in hgStations) {
	...
	function GetProducingPlaces(cargo) {
		if(!cargoProducingPlaces.rawin(cargo)) { // ???????????????????????????????????????
	...
	function GeAcceptingPlaces(cargo) {
		if(!cargoAcceptingPlaces.rawin(cargo)) { // ???????????????????????????????????????
	...
	function GetPlaces(cargo, isProducing) {
		...
		foreach(station in hgStations) {
			...
			if(station instanceof PlaceStation) { // PlaceStation?CoverageTile?????
	...
	function GetAcceptIndustryId(cargo, platformRectangle, coverageTileList) {
		...
		// TODO: ???????id??xy?????????truncate???????xy???????????
		local xy = platformRectangle.GetCenter().tile; //??...
	...
	function GetCoverageTileList() {
		if(coverageTileList == null) {
			...
			if(SquirrelAI.Get().openttdVersion >= 14) {
	...
	function IsBestEngineSetUsingSrouce(currentEngineSet) {
		...
		/*if(IsBiDirectional()) {
			??????????????????water?dest?full load??????dest??????rate?????????
...
class StationFactory {
	...
	function CreateBest(target, cargo, toTile, useStationGroup = true) {
		...
		if(result == null && place instanceof TownCargo && place.IsAccepting() && !SquirrelAI.Get().IsAvoidExtendCoverageAreaInTowns()) {
	...
	function GetBestHgStationCosts(hgStations, fromTile, toTile, stationOrg, cargo, useStationGroup) {
		...
		if(this instanceof RailStationFactory && considerAcceptance && SquirrelAI.Get().CanExtendCoverageAreaInTowns()) {
			acceptanceScoreMultiplier = 12;
		}
		local considerProduction = place != null && !considerAcceptance && place.IsProducing();
		if(this instanceof RoadStationFactory && SquirrelAI.Get().IsDistantJoinStations()) {
		...
		foreach(stationIndex, station in hgStations) {
			...
			station.cargo = cargo; // WaterStation.GetBuildableScore()???
			station.place = place;
	...
	function SelectBestWithPieceStation(place, cargo, toTile) {
		...
		if(result != null) {
			result.stationGroup = null; //virtualStationGroup????????
...
class RoadStationFactory extends StationFactory {
	...
	constructor(cargo,isPieceStation = false,engineSet = null) {
		...
		if(isPieceStation) {
			...
			this.platformNum = SquirrelAI.Get().roiBase || CargoUtils.IsPaxOrMail(cargo) ? 1 : 2;
			this.platformLength = 1;
			if(engineSet != null) {
				this.platformLength = AIEngine.IsArticulated(engineSet.engine) ?  2 : 1;
			}
			this.buildSupportRoad = SquirrelAI.Get().IsDistantJoinStations();
...
class SrcRailStationFactory extends RailStationFactory {
	...
	function GetPlatformNum() {
		if(useSingle) {
			return 1;
		} else if(useSimple || SquirrelAI.Get().IsInfrastructureMaintenance()) {
			return 2;
		} else {
			return 3;
		}
	...
	function Create(platformTile,stationDirection) {
		if(useSimple || useSingle) {
			...
			result.useDepot = !useSingle && SquirrelAI.Get().IsEnableVehicleBreakdowns();
			return result;
		} /*else if(SquirrelAI.Get().IsInfrastructureMaintenance()) {
...
class TerminalStationFactory extends RailStationFactory {
	...
	function GetPlatformNum() {
		...
		if(useSingle) {
			...
			return 2;
		} else if(SquirrelAI.Get().IsDistantJoinStations()) {
			return 2;
		} else {
			return 2;
		}
...
class HgStation {
		...
		foreach(stationGroup in place.GetStationGroups()) {
			...
			if(isRoad && isAccepting) {
				foreach(usingRoute in stationGroup.GetUsingRoutesAsSource()) {
					if(usingRoute.GetVehicleType() == AIVehicle.VT_ROAD) ng = true; // ROAD?src?????????dest?????????
				}
			}
	...
	function BuildPlatform(isTestMode, supressWarning = false) {
		...
		if(IsJoin()) {
			if(!SquirrelAI.Get().IsDistantJoinStations()) {
				joinStation = AIStation.STATION_JOIN_ADJACENT; // distant_joint_stations????STATION_JOIN_ADJACENT???????join?????
				if(ExistsStationGroupsMoreThanOneAround()) {
					if(!isTestMode && !supressWarning) {// IsDistantJoinStations==false???????????????join????
		...
		for(local i=0;; i++) {
			if(BuildStationSafe(joinStation, isTestMode)) {
				...
				if(place != null && cargo != null && place instanceof HgIndustry && place.IsAccepting()) {
					...
					foreach(industry in SearchIndustries(cargo,false)) { //BUG: ???stationGroup?????????test?????????
						if(industry < place.industry) { // ?????industry???cargo?????????industryId????????
	...
	function BuildSpreadPieceStations(notAccepted = false) {
		...
		local area = Rectangle.Center(platform.GetCenter(), min(63,SquirrelAI.Get().maxStationSpread)).GetTileList();
		...
		foreach(tile,_ in area) {
			...
			if(BuildPieceStation(tile, place 
					/*?town?????production???????TownCargo( AITile.GetClosestTown(tile), cargo, true )*/ 
...
/*		
	...
	function BuildExec() {
		...
		if(!builded) {
			SquirrelAI.WaitForMoney(GetNeedMoney());
		...
		if(isTownPlace) {
			if(SquirrelAI.Get().CanExtendCoverageAreaInTowns() && stationGroup.hgStations.len() == 1 && CargoUtils.IsPaxOrMail(cargo)){
				local success = false;
				...
				if(CargoUtils.IsPaxOrMail(cargo) && this instanceof RailStation) {
					foreach(paxOrMail in SquirrelAI.Get().GetPaxMailCargos()) {
						if(TownBus.CanUse(paxOrMail)) {
							BuildPieceStationTo(place, paxOrMail); // ???
						}
					}
				}
			}
		...
		foreach(corner in GetPlatformRectangle().GetCorners()) { 
			HgStation.townUsed.rawset(AITile.GetTownAuthority(corner.tile),true); // TODO: ??????
		}		

		return true;
	}
	
	// WaterStation/RoadStation?override
	function CheckBuildTownBus() {
		foreach(corner in GetPlatformRectangle().GetCorners()) { 
			TownBus.Check(corner.tile, null, SquirrelAI.Get().GetPassengerCargo(), null);
		}
		local growthTown = place != null && place instanceof TownCargo && place.CanGrowth();
		if(SquirrelAI.Get().IsDistantJoinStations() && !growthTown) {
			return;
		}
		if(growthTown) {
	...
	function PurchaseLandTile(tile) {
		if(AIObjectType.BuildObject(3,0,tile)) {
			SquirrelAI.Get().maybePurchasedLand.rawset(true,0);
		}
	}
	
	function PurchaseLand(tile) {
		return; // ????????????
		if(SquirrelAI.Get().HasIncome(100000)) {
	...
	function InitializeName() {
		if(SquirrelAI.Get().IsDisabledPrefixedStatoinName()) {
			if(name != null) {
				AIStation.SetName(stationId, name); // ?????busstop??????
			}
			return;
		}
		local s = "";
	...
	function SearchIndustries(cargo, isProducing) {
		...
		foreach(industry,_ in industryList) {
			...
			foreach(tile in hgIndustry.GetTiles(radius, cargo)) { // TODO: isProducing==false?????????industryId?????
	...
	function CanShareByMultiRoute(infrastractureType,cargo) {
		return true; // CommonRouteBuilder??????
	}
	
	function Share() {
		return true; //RoadStation?override
	}
...
class RailStation extends HgStation {
	...
	function IsBuildablePreCheck() {
		local rect = GetPlatformRectangle();
		if(SquirrelAI.Get().IsAvoidRemovingWater()) {
...
/*		local platformRect = GetPlatformRectangle();
		return SquirrelAI.IsBuildable(platformRect.lefttop.tile) && SquirrelAI.IsBuildable(platformRect.rightbottom.tile - AIMap.GetTileIndex(1,1));*/
	}
...
class RoadStation extends HgStation {
	...
	constructor(platformTile, platformNum, platformLength, buildSupportRoad, stationDirection, stationType, isTram) {
		...
		if(stationDirection == HgStation.STATION_SE) {
			originTile = MoveTile(platformTile, 0, 0); //??????platform??????????????
		...
		if(isTestMode) {
			...
			if(!result) {
				...
				if(lastError == AIError.ERR_AREA_NOT_CLEAR 
						|| lastError == AIError.ERR_VEHICLE_IN_THE_WAY // ???????????????????????????
						|| lastError == AIRoad.ERR_ROAD_DRIVE_THROUGH_WRONG_DIRECTION
						|| lastError == AIRoad.ERR_UNSUITABLE_ROAD // Conveyer Belt???????????????????
						|| lastError == AIError.ERR_UNKNOWN/*??????????????*/) {
					return false;
				}
...
//		HgLog.Warning("Build levelTiles:"+levelTiles+" isTestMode:"+isTestMode);
		...
		if(levelTiles) {
			if(isTestMode) {
				foreach(tile in GetTiles()) {
					if(SquirrelAI.IsBuildable(tile)) continue;
					if(AIRoad.IsRoadTile(tile)) {
						...
						if(roadType == null) continue; // tram????road??????
		...
		GetRectangle(0,-1, GetRoadNum(),platformLength+1).LevelTiles(null, isTestMode); // pathfind??????????track???????????
...
/*		local result = 0; //TODO: ???flat isbuildable????
		
		foreach(tile in GetTiles()) {
			if(SquirrelAI.IsBuildable(tile)) {
				result ++;
				if(IsFlat(tile)) {
					result ++;
				}
	...
	function CheckBuildTownBus() {
		if(SquirrelAI.Get().CanExtendCoverageAreaInTowns()) {
			return;
		}
		...
		if(isTestMode) {
			if(!AIRoad.BuildDriveThroughRoadStation (platformTile, platformTile + HgTile.DIR4Index[roadDir],roadVehicleType , joinStation)) {
				...
				if(AIError.GetLastError() == AIError.ERR_LOCAL_AUTHORITY_REFUSES) {
					...
					if(IsJoin()) {
						if(SquirrelAI.Get().IsDistantJoinStations()) return true;
...
/*				if(AIError.GetLastError() == AIError.ERR_AREA_NOT_CLEAR || AIError.GetLastError() == AIError.ERR_UNKNOWN) {
				...
				if(!AIRoad.BuildDriveThroughRoadStation (platformTile, platformTile + HgTile.DIR4Index[roadDir], roadVehicleType, joinStation) ) {
					...
					if(AIError.GetLastError() == AIError.ERR_AREA_NOT_CLEAR //TODO: OK???????????????????
							|| AIError.GetLastError() == AIError.ERR_VEHICLE_IN_THE_WAY // TODO: ???????
							|| AIError.GetLastError() == AIRoad.ERR_ROAD_DRIVE_THROUGH_WRONG_DIRECTION
							|| AIError.GetLastError() == AIError.ERR_UNKNOWN/*??????????????*/
							|| AIError.GetLastError() == AIError.ERR_FLAT_LAND_REQUIRED
							|| AIError.GetLastError() == AITunnel.ERR_TUNNEL_CANNOT_BUILD_ON_WATER /*??????????????????*/) { 
						return false; //AIError.ERR_LOCAL_AUTHORITY_REFUSES?????????????????????????????
					}
				}
			}
			return true;
		...
		if(AITile.GetSlope(platformTile) == AITile.SLOPE_FLAT && GetTownRating(platformTile) >= AITown.TOWN_RATING_VERY_GOOD && IsDemolishableTownBuilding(platformTile)) {
			local execMode = AIExecMode(); //?????????????
			if(!supressWarning ) {
				HgLog.Warning("demolish TownBuilding "+HgTile(platformTile)); //???2?????
			}
...
/*		yield [-1,platformLength];
	...
	function Build(levelTiles=true, isTestMode=true) {
		...
		while((xy = resume tilesGen) != null) {
			if(!SquirrelAI.IsBuildable(At(xy[0],xy[1]))) {
				if(!isTestMode) {
...
//		return [At(3,platformLength+4),At(2,platformLength+4),At(1,platformLength+5),];
	...
	function RemoveDepots() { //TrainRouteExtendBuilder??????
	...
	function GetTiles() {
		...
		return tiles; // RemoveDepots??????depot?tile??????????
	}
...
class DestRailStation extends RailStation {
	...
	function Build(levelTiles=false, isTestMode=true) {
		...
		foreach(rail in rails) {
			if(!SquirrelAI.IsBuildable(At(rail[1][0],rail[1][1]))) {
				if(!isTestMode) {
...
//		BuildDepot(At(platformNum,0),At(platformNum-1,0));
	...
	function GetEntranceRooms() {
		...
		for(local i=-1; i<=platformNum; i++) {
			for(local j=platformLength+2; j<=platformLength+3;j++) {
				if(SquirrelAI.IsBuildable(At(i,j))) {
					result ++;
				}
			}
		}
		result -= platformNum;
		for(local i=0; i<platformNum; i++) {
			if(SquirrelAI.IsBuildable(At(i,-1))) {
				result ++;
			}
		}
		for(local i=-1; i<=platformLength+1; i++) {
			if(SquirrelAI.IsBuildable(At(-1,i))) {
				result ++;
			}
			if(SquirrelAI.IsBuildable(At(platformNum,i))) {
				result ++;
			}
		}
	...
	function GetEntranceFlats() {
		...
		for(local i=-1; i<=platformNum; i++) {
			for(local j=platformLength+2; j<=platformLength+3;j++) {
				if(IsFlat(At(i,j)) && SquirrelAI.IsBuildable(At(i,j))) {
					result ++;
				}
			}
		}
		result -= platformNum;
		for(local i=0; i<platformNum; i++) {
			if(IsFlat(At(i,-1)) && SquirrelAI.IsBuildable(At(i,-1))) {
				result ++;
			}
		}
		return result;
	...
	function At(x,y) {
		local result = HgStation.At(platformNum-1-x,(platformLength+2)-y); //???????????
		if(result == null) {
...
class RealSrcRailStation extends RailStation {
	...
	*/
	...
	constructor(platformTile, platformLength, stationDirection) {
		...
		if(stationDirection == HgStation.STATION_SE) {
			originTile = MoveTile(platformTile, platformNum - 3, 0); //??????platform??????????????
		} else if(stationDirection == HgStation.STATION_NW) {
...
class SimpleRailStation extends RailStation {
	...
	function Save() {
		local t = HgStation.Save();
		t.useDepot <- useDepot; // rail update?????
		return t;
	}
	...
	function Build(levelTiles=false, isTestMode=true) {
		...
		TileListUtils.LevelAverage(GetTileListForLevelTiles(tiles), null, isTestMode, null, true); // pathfind??????????track???????????

...
class HgTable {
	...
	static function Diff(table1, table2) {
		...
		foreach(k,v in table1) {
			...
			result.remove.rawset(k,v); // table1????table2???
		}
		foreach(k,v in table2) {
			if(table1.rawin(k)) continue;
			result.append.rawset(k,v); // table2????table1???
		}
		return result;
	}
...
class SortedList {
	...
	constructor(valuator) {
		...
		this.list = AIList();
		this.list.Sort( AIList.SORT_BY_VALUE, false ); // ???????
		this.arr = [];
	}
...
class BuildUtils {
	...
	static function CheckCost(func) {
		...
		if(SquirrelAI.Get().IsTooExpensive(cost)) {
			return false;
		}
		return true;
	}
	...
	static function WaitForMoney(func) {
		...
		while(true) {
			if(!SquirrelAI.WaitForPrice(cost)) {
				return false;
			}
			local r = func();
			if(!r && AIError.GetLastError() == AIError.ERR_NOT_ENOUGH_CASH) { // ??????????????????
	...
	static function WaitForMoneyValid(func,valid) {
		...
		while(true) {
			...
			local r = func();
			if(!valid(r) && AIError.GetLastError() == AIError.ERR_NOT_ENOUGH_CASH) { // ??????????????????
				cost += SquirrelAI.Get().GetInflatedMoney(10000);
				continue;
			}
			return r;
	...
	static function BuildVehicleWithRefitSafe(a,b,c) {
		...
		while(true) {
			...
			local r = func();
			if(!AIVehicle.IsValidVehicle(r) && AIError.GetLastError() == AIError.ERR_NOT_ENOUGH_CASH) { // ??????????????????
				cost += SquirrelAI.Get().GetInflatedMoney(10000);
				continue;
			}
			return r;
	...
	function PlantTreeTown(town, goalRating) {
		...
		townPlantTreeDate.rawset(town, AIDate.GetCurrentDate()); // ??????????????????????????????
		return false;
	}
...
class VehicleUtils {
	...
	static function GetCargoWeight(cargo, quantity) { // ???
		...
		if (AICargo.IsFreight(cargo)) {
			result *= SquirrelAI.Get().GetFreightTrains();
		}
		return result;
	}
	...
	static function GetCommonCargoWeight(cargo, quantity) {
		if(SquirrelAI.Get().openttdVersion >= 13) {
	...
	static function GetSlopeForce(slopedWeight, totalWeight) {
		return slopedWeight * SquirrelAI.Get().GetTrainSlopeSteepness() * 100 + totalWeight * 10 + totalWeight * 15;
	}
	...
	static function GetRoadSlopeForce(weight) {
		return weight * SquirrelAI.Get().GetRoadvehSlopeSteepness() * 100 + weight * 10 + weight * 75;
	}
	...
	static function AdjustTrainScoreBySlope(score, engine, start, end, isBiDirectional = false) {
		...
		local slopeSteepness = SquirrelAI.Get().GetTrainSlopeSteepness();
		if(AIEngine.GetMaxTractiveEffort(engine) < slopeSteepness * 50) {
			considerSlopeLevel = 2;
		} else if(SquirrelAI.Get().mountain && AIEngine.GetPower(engine) < slopeSteepness * 600) {
	...
	static function GetDays(distance, speed) {
		return max(1, distance * 664 / speed / 24 / SquirrelAI.Get().GetDayLengthFactor());
	}
	
	static function GetSpeed(distance, days) {
		return distance * 664 / (days * SquirrelAI.Get().GetDayLengthFactor() * 24);
	}

	static function GetDistance(speed, days) {
		return speed * (days * SquirrelAI.Get().GetDayLengthFactor() * 24) / 664;
	}
...
class CargoUtils {
	...
	static function GetStationRate(cargo, maxSpeed) { // 255 == 100%
		...
		if(SquirrelAI.Get().ecs) {
			if(!CargoUtils.IsPaxOrMail(cargo) && result < (SquirrelAI.Get().IsRich() ? 153 : 179)) {
				result /= 4; // 70%???????????
			}
		}
		return result;
	...
	static function GetStationRate(maxSpeed) {
		...
		//stationRate += 33; // ?????
		stationRate += SquirrelAI.Get().IsRich() ? 26 : 0; //??
		return stationRate;
	}
	...
	static function GetStationRateWaitTimeFullLoad(prodictionPerMonth, initialRate, capacity, maxSpeed) {
		...
		local t0 = abs(endRate - iniRate) / a; // rate???????????
		local c0 = (iniRate + endRate) / 2 * t0 * prodPerDay ; // t0??????production
		if(capacity < c0) {
			local iniProd = iniRate * prodPerDay;
			//local aProd = a * prodPerDay;
			//local t = (-iniProd+pow(iniProd * iniProd + 2 * aProd * capacity, 0.5)) / aProd;
			local t = capacity / (prodPerDay * iniRate); // ?????????
			...
			local t = t0 + (capacity - c0) / (endRate * prodPerDay); // rate???????? + ????????????????
	...
	static function GetStationRateStock(cargo, production, initialRate, vehicleType, maxSpeed, iniIntervalDays) {
		...
		foreach(d in [[7,130],[8,95],[15,50],[22,25],[1000000,0]]) {
			...
			local rate = stationRate + d[1] + CargoUtils.GetStockStationRate(result); // stock?????????????rate?????????
			local truncate = 0;
			if(rate <= 64) { // ???????????????
				if(result >= 200) {
					truncate = 3;
			...
			local reachDay = abs(oldRate - rate) * 5 / 4; // 2.5??????2??
			if(day < reachDay) {
			...
			if(SquirrelAI.Get().ecs && !CargoUtils.IsPaxOrMail(cargo)) {
				if(rate < 179/*70%*/) {
					receive /= 4; // ecs??70%???????????????
				}
			} else {
				if(AICargo.HasCargoClass(cargo,AICargo.CC_BULK) && rate >= 204/*80%*/) {
					receive = receive * 15 / 10; // ?????80%????????????????
				}
			}
	...
	static function IsPaxOrMail(cargo) {
		foreach(c in SquirrelAI.Get().GetPaxMailCargos()) {

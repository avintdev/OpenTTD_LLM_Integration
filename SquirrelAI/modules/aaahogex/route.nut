...
class Route {
	...
	static function Estimate(vehicleType, cargo, distance, production, isBidirectional, infrastractureTypes=null) {
		...
		local estimateTable = SquirrelAI.Get().estimateTable;
		local productionIndex = SquirrelAI.Get().GetEstimateProductionIndex(production);
		local distanceIndex = SquirrelAI.Get().GetEstimateDistanceIndex(distance);
		...
		if(!estimateTable.rawin(key)) {
			...
			estimator.distance = SquirrelAI.distanceEstimateSamples[distanceIndex];
			estimator.production = SquirrelAI.productionEstimateSamples[productionIndex];
	...
	static function GetPaxMailTransferBuildingCost(cargo) {
		if(CargoUtils.IsPaxOrMail(cargo)) {
			if(!SquirrelAI.Get().IsDistantJoinStations() || SquirrelAI.Get().IsAvoidExtendCoverageAreaInTowns()) {
				local pax = SquirrelAI.Get().GetPassengerCargo();
				if(TownBus.CanUse(pax)) {
					local busPrice = TownBus.busPrice[pax];
					return busPrice * 8 + SquirrelAI.GetInflatedMoney(2000); // ????????
				}
			} else {
				return SquirrelAI.GetInflatedMoney(5000);
			}
		}
		return 0;
	}
	...
	function _IsOverflow( cargo = null, isDest = false, callRoutes = null ) {
		...
		local bottom = max(1000, min(capacity * (SquirrelAI.Get().roiBase ? 3 : 20), 3000)); //TODO:?????????
	...
	function _NeedsAdditionalProducingCargo(cargo, callRoutes = null, isDest = false) {
		...
		if(!IsTransfer()) {
			...
			local needs = false; // ????????isDest=false?????(BiDirectional????accept???)
	...
	function GetLeftCapacity(cargo, isDest = false, callRoutes = null) {
		...
		if( IsReturnRoute(isDest) ) {
			...
			/*dest?????????local vehicleList = GetVehicleList();
	...
	// TrainReturnRoute?override???
	function IsReturnRoute(isDest) {
		return isDest;
	}
	
	function NeedsToMeetDestDemandPlace(place) {
		if(SquirrelAI.Get().ecs && place != null && place instanceof HgIndustry) {
			return true;
		}
		return false;
	}
	...
	function GetDelivableProduction(cargo, callers = null, isFuture = false) {
		...
		/*if(currentCapacity == 0 && GetVehicleType() == AIVehicle.VT_RAIL) {
			currentCapacity = GetRouteCapacity() * 100; // TODO: ????????????????
		}*/		
	...
	function GetCargoIsTransfered() {
		local result = {};
		foreach(cargo in GetCargos()) {
			if(srcHgStation.stationGroup.IsCargoTransferToHere(cargo)) { // TODO: bidirectional???
	...
	function IsValidDestStationCargo() {
		...
		if(engineCargos.len() == 0) { // latestEngineSet????
	...
	function CalculateSubCargos() {
		productionCargoCache.clear(); // subcargo????????????
		...
		if(IsBiDirectional()) {
			...
			foreach(subCargo,_ in AICargoList()) {
				...
				if(SquirrelAI.Get().IsManyTypesOfFreightAsPossible()) {
			...
			foreach(subCargo,_ in AICargoList()) {
				...
				if(isShort && !isTransfer && destHgStation.place!=null) {
					if(!destHgStation.place.IsAcceptingCargo(subCargo) && !dest.IsCargoDeliverFromHere(subCargo)) { // ????????transfer??????????
						continue;
					}
				}
	...
	function IsDestDest(destRoute = null) {
		...
		if(IsTransfer() && destRoute.IsBiDirectional() && destHgStation.stationGroup == destRoute.destHgStation.stationGroup) {
			...
			return false;//TODO !isTransfer ?place??????
		}
	}
	...
	function CheckClose() {
		...
		if(IsTransfer()) {
			...
			local isRemove = SquirrelAI.Get().IsInfrastructureMaintenance();
			...
			if(!acceptedCargo) {
				...
				if(destHgStation.place == null) {
					// destHgStation?share?????place==null???????
					//?????place???????????????????????Route?place??????????????
					HgLog.Warning("Route Remove (destStation.place == null && not accept cargo)"+this); 
					Remove();
					return;
				} else if(SquirrelAI.Get().IsDistantJoinStations() && destHgStation.place instanceof TownCargo && CargoUtils.IsPaxOrMail(cargo)) {
			...
			if(!IsSupportRaw() && (GetVehicleType() == AIVehicle.VT_ROAD || IsSingle())) {
				...
				foreach(route in routes) {
					...
					/*???????
					if(route.IsSameSrcAndDest(this)) {// industry??supply?????(for FIRS)
						continue;
					}*/
...
class CommonRoute extends Route {
	...
	function GetEngineCapacity(self, engine, cargo) {
		...
		if(self.GetVehicleType() == AIVehicle.VT_ROAD && AIEngine.IsArticulated(engine)) {
			return  AIEngine.GetCapacity(engine); // Articulated?????GetBuildWithRefitCapacity?????????
		}*/
		...
		if( self.GetVehicleType() == AIVehicle.VT_AIR) {
			if(cargo == SquirrelAI.GetPassengerCargo()) {
	...
	static function CalculateVehiclesSpeed(vehicleType) {
		...
		SquirrelAI.Get().WaitDays(3,true);
	...
	static function CheckOldVehicles() {
		if(!SquirrelAI.Get().IsEnableVehicleBreakdowns()) {
			return;
		}
	...
	static function CheckReduce(self,emergency = false) {
		...
		local reduceForMaintenance = false; // ????????????????? SquirrelAI.Get().IsInfrastructureMaintenance() && SquirrelAI.Get().noRouteCnadidates;
		...
		if(vehicleType == AIVehicle.VT_AIR) {
			if(year % 10 == 9) {
				Place.canBuildAirportCache.clear(); //10??1?????????
			}
		}

		local engineList = AIEngineList(vehicleType);
		engineList.Valuate(AIEngine.GetDesignDate);
		engineList.KeepAboveValue(AIDate.GetCurrentDate() - 365*2); //??????????1????
		...
		foreach(index, route in routeInstances) {
			/*if(route.IsClosed()) { ????????????????????IsInfrastractureMaintenance
				continue;
			}*/
			if(route.IsRemoved()) {
				continue;
			...
			if(route.profits.len() >= checkYear) {
				...
				averageProfit = sum / d; // TODO: ???????????????? - totalValue * 9 / 100/*????*/;
				if(averageProfit <= 0) {
		...
		if(speedCount >= 100) {
			SquirrelAI.Get().roadTrafficRate = speedRateSum / speedCount;
			HgLog.Info("roadTrafficRate:"+SquirrelAI.Get().roadTrafficRate);
		}
		
		if(((tooManyVehicles || reduceForMaintenance) && minRoutes.Count() >= 10) || emergency) {
			local removeNum = (minRoutes.Count() + 32) / 33; // 3%???
	...
	function CheckReduceForRoadTransfer(vehiclesSpeed) { // transfer???????????????????
	...
	isClosed = null;
	isRemoved = null;
	isWaitingProduction = null; // ??cargo??????route????
	isWaitingDestRoute = null; // ??destroute????????route????
	...
	function BuildDepot(path) {
		...
		if(depot == null && ("GetDepot" in destHgStation)) { // ??dest???
		...
		while(true) {
			...
			if(vehicle==null || !AIVehicle.IsValidVehicle(vehicle)) {
				if(AIError.GetLastError() == AIError.ERR_NOT_ENOUGH_CASH) {
					cost += SquirrelAI.Get().GetInflatedMoney(10000);
					continue;
				}
	...
	function MakeOrder(vehicle) {
		...
		if(useDepotOrder && (SquirrelAI.Get().IsEnableVehicleBreakdowns() || GetVehicleType() == AIVehicle.VT_ROAD)) {
		...
		if(useServiceOrder && SquirrelAI.Get().IsEnableVehicleBreakdowns()) {
			AIOrder.AppendOrder(vehicle, depot, AIOrder.OF_SERVICE_IF_NEEDED + nonstopIntermediate );
		}
		
		AppendSrcToDestOrder(vehicle);
		
		if(destDepot != null && (SquirrelAI.Get().IsEnableVehicleBreakdowns() || GetVehicleType() == AIVehicle.VT_ROAD)) {
		...
		if(useServiceOrder && destDepot != null && SquirrelAI.Get().IsEnableVehicleBreakdowns()) {
	...
	function AppendRemoveOrder(vehicle) {
		...
		if(!AIVehicle.SendVehicleToDepot(vehicle)) {
			...
			if(lastError == AIError.ERR_UNKNOWN) { // ?????
	...
	function CloneVehicle(vehicle) {
		...
		if(this instanceof RoadRoute && IsBiDirectional() && destDepot != null) {
			if(GetNumVehicles() % 2 == 1) {
				depot = destDepot;
				skipToOrder = SquirrelAI.Get().IsEnableVehicleBreakdowns() ? AIOrder.GetOrderCount(vehicle) - 2 : 2;
			}
		}
		...
		while(true) {
			if(SquirrelAI.Get().IsInfrastructureMaintenance() && SquirrelAI.Get().GetUsableMoney() < cost) {
	...
	function ChooseEngineSet() {
		...
		if(latestEngineSet == null 
			...
			local production = max(50,GetProduction()); // ????src????????????
			local engineSet;
			if(!SquirrelAI.Get().roiBase && IsTownTransferRoute()) { // ??????
			...
			if(engineSet == null) {
				...
				return latestEngineSet; //null????route????????????????????CheckReduce??????????
			}
			...
			latestEngineSet.productionIndex <- SquirrelAI.Get().GetEstimateProductionIndex(production);
	...
	function GetMaxVehicles() {
		...
		local cruiseSpeed = max( 4, AIEngine.GetMaxSpeed(engine)
				* ( 100 + (SquirrelAI.Get().IsEnableVehicleBreakdowns() ? AIEngine.GetReliability(engine) : 100)) / 200 );
	...
	function CheckNotProfitableOrStopVehicle( emergency = false, tooMany = false ) {
		...
		if(checkProfitable) { // transfer???????????????????? TODO:???????????????????
			foreach(vehicle,v in vehicleList) {
				...
				if(notProfitable) {
					...
					//AIVehicle.SendVehicleToDepot (vehicle);
					AppendRemoveOrder(vehicle);
					if(vehicleType == AIVehicle.VT_ROAD) { //???????????????NeedsAdditionalProducing==false??????????????????????ROAD????
						maxVehicles = min(vehicleList.Count(), maxVehicles);  // TODO: ????????????????????????????????????????????????
		...
		} else if(vehicleType == AIVehicle.VT_AIR) { //VT_ROAD?srcStation????stop??
			local reduce = 0;
			if(tooMany) {
				local list = AIList();
...
/*				
	...
	function _CheckBuildVehicle() {
		...
		local townTransfer = IsTownTransferRoute();
		if(townTransfer && AIBase.RandRange(100)<80) { // towntransfer??????????????????
			return;
		}
...
/*			if(!(isRemoved || isClosed || tooMany)) {
		...
		local enableVehicleBreakDowns = SquirrelAI.Get().IsEnableVehicleBreakdowns();
...
/*	|| (lastCheckProductionIndex != null && lastCheckProductionIndex != SquirrelAI.Get().GetEstimateProductionIndex(GetProduction())))) {???????????????*/
		...
		if(AIBase.RandRange(100) < (SquirrelAI.Get().buildingTimeBase ? 5 : 25) * (townTransfer ? 1 : 1)) {
		...
		if(isDestOverflow) {
			...
			if(tooMany || AIBase.RandRange(100) < vehicleList.Count()) {
				...
				if(SquirrelAI.Get().buildingTimeBase && (!tooMany || AIBase.RandRange(100) < vehicleList.Count())) {
		...
		if(AIBase.RandRange(100) < (SquirrelAI.Get().roiBase ? 3 : 15)) {
		...
		if( AIBase.RandRange(100) < 10 && CargoUtils.IsPaxOrMail(cargo)) { // ???????????????
			//local c4 = PerformanceCounter.Start("c4");	
			if(!IsOverflow(cargo,false)) { //NeedsAdditionalProducing?town????production???????????????false???????
		...
		//local bottomWaiting = max(min(200, SquirrelAI.Get().roiBase ? capacity * 6 : capacity), 10);
		/*
		if(SquirrelAI.Get().roiBase && IsBiDirectional() && AIStation.GetCargoWaiting(srcHgStation.GetAIStation(),cargo) < bottomWaiting && vehicleList.Count()>=4) {
			return;
		}*/
		
		//local c6 = PerformanceCounter.Start("c6");	
		local needsProduction = (!tooMany && vehicleList.Count() < 10) ? (SquirrelAI.Get().roiBase ? 30 : 10) : 100; //= min(4,maxVehicles / 2 + 1) ? capacity : bottomWaiting;
		if(townTransfer) needsProduction = max(100,needsProduction);
		if(showLog) {
			HgLog.Info("needsProduction "+needsProduction+" "+this);
		}
		/*
		if(SquirrelAI.Get().roiBase && isBiDirectional) {
		...
		if(cargoWaiting > needsProduction || (vehicleList.Count()==0 && (!isTransfer || needsAddtinalProducing))) {
			...
			if(!ExistsWaiting(vehicles)) {
				local latestVehicle = null; //??GetLatestVehicle();
				foreach(v,_ in vehicleList) {
				...
				if(latestVehicle == null) {
					...
					if(cargoWaiting > capacity / 4 && GetVehicleType() != AIVehicle.VT_ROAD && AIStation.GetCargoRating(srcHgStation.stationId, cargo) < 50
						/* && SquirrelAI.Get().IsInfrastructureMaintenance()*/) {
						buildNum = max(1, buildNum);
					}
					if(townTransfer) {
					...
					if(buildNum >= 1) {
						...
						foreach(v,_ in inDepot) {
							if(choosenEngine == AIVehicle.GetEngineType(v) 
									&& (!SquirrelAI.Get().IsEnableVehicleBreakdowns() || AIVehicle.GetAgeLeft(v) >= 1000)) {
								AIVehicle.StartStopVehicle(v);
						...
						if(buildNum >= 1 && depot != null) {
							foreach(v,_ in AIVehicleList_Depot(depot)) { // ???depot???group????vehicle?????
						...
						if(latestEngineSet.price > 0) {
							buildNum = min(SquirrelAI.Get().GetUsableMoney() / latestEngineSet.price ,buildNum);
						}
...
/*			
	...
	function RemoveFinished() {
		...
		if(SquirrelAI.Get().IsInfrastructureMaintenance()) {
			Demolish();
		}
...
//		}
	...
	function ReOpen() {
		...
		saveData.maxVehicles = maxVehicles;
		//HgLog.Info("maxVehicles:"+maxVehicles+" "+this);
		//PlaceDictionary.Get().AddRoute(this); RemoveRoute??????AddRoute??????
...
class Construction {
	...
	constructor(params=null) {
		if(params != null) { // params!=null??load??Load()?????
	...
	function StartConstraction() {
		if(SquirrelAI.Get().loadData != null && "constructions" in SquirrelAI.Get().loadData) { // ????????Constraction???????loadData???????????
			SquirrelAI.Get().loadData.constructions.push(saveData);
		} else {
			SquirrelAI.Get().constructions.push(saveData);
		}
	}
	
	function EndConstraction() {
		if(SquirrelAI.Get().loadData != null && "constructions" in SquirrelAI.Get().loadData) {
			SquirrelAI.Get().loadData.constructions.pop();
		} else {
			SquirrelAI.Get().constructions.pop();
		}
	}
...
class RouteModificatin extends Construction {
	...
	function Rollback() {
		...
		if(saveData.oldBuilding != null) {
			// route.SetBuilding(saveData.oldBuilding); route.isBuilding?load?false???????????
		}
	}
}
...
class RouteBuilder extends Construction {
	...
	function Build() {
		...
		if(srcPlace != null && !GetOption("notNeedToMeetDemand",false)) {
			...
			if(needsToMeetDemand && currentProduction < 50 && srcPlace.GetAccepting().GetRoutesUsingDest().len() == 0) {
				// ?????????????????????????????
				foreach(route in SquirrelAI.Get().SearchAndBuildToMeetSrcDemandMin( srcPlace, null, 
						{capacity = 1}, 
	...
	function DoPostBuild() {
		...
		builtRoute.isBuilding = true;// result?route?????????????(WaterRouteBuilder.BuildCompoundRoute)
	...
	function _DoPostBuild(builtRoute) {
		...
		local routeCandidates = GetOption("setRouteCandidates", false) ? SquirrelAI.Get().routeCandidates : null;
		local notNeedToMeetDemand = GetOption("notNeedToMeetDemand",false);
		local limitDate = GetOption("limitDate",null);
		local vehicleType = GetVehicleType();
		
		if(searchTransfer && GetOption("canChangeDest",true) && !IsTransfer()) { // ??????
			if(!builtRoute.cannotChangeDest && vehicleType == AIVehicle.VT_RAIL) {
				SquirrelAI.Get().SearchAndBuildAdditionalDestAsFarAsPossible(builtRoute, true);
			}
		}
		if(limitDate != null && limitDate < AIDate.GetCurrentDate()) { // ???????????????????????
			HgLog.Warning("limitDate(DoPostBuild) "+builtRoute);
			return;
		}
		if(routeCandidates != null) {
			builtRoute.ChooseEngineSet(); // engineSet??????
			if(!notNeedToMeetDemand && srcPlace != null) {
				if(CheckClose()) {
					return;
				}
				foreach(routeId in saveData.preBuiltRoutes) { // ????????????
					...
					routeCandidates.Extend( ShowPlansLog( SquirrelAI.Get().GetTransferCandidates( route, {} )));
				}
				HgLog.Info("DoPostBuild GetMeetPlacePlans srcPlace:"+srcPlace+" builtRoute:"+builtRoute);
				routeCandidates.Extend( ShowPlansLog( SquirrelAI.Get().GetMeetPlacePlans( srcPlace, builtRoute ) ) );
				
			}
			if(searchTransfer) {
				HgLog.Info("DoPostBuild GetTransferCandidates builtRoute:"+builtRoute);
				routeCandidates.Extend( ShowPlansLog( SquirrelAI.Get().GetTransferCandidates( builtRoute, 
			...
			if(isBiDirectional) {
				...
				local transferCandidates = SquirrelAI.Get().GetTransferCandidates( builtRoute, 
					{ destOnly = true} )
				...
				if(dests.len() >= 1) {
					...
					if(destPlace != null && destPlace.IsIncreasableInputCargo(cargo)) {
						...
						foreach(prodCargo in producing.GetCargos()) {
							...
							routeCandidates.Extend( ShowPlansLog( SquirrelAI.Get().CreateRoutePlans( 
		...
		foreach(r in shareRoute) {// ????????????????(??enginering supply??????)
	...
	function ShowPlansLog( plans ) {
		foreach(e in SquirrelAI.Get().GetSortedRoutePlans(plans).GetAll()) {
...
class CommonRouteBuilder extends RouteBuilder {
	...
	function DoBuild() {
		...
		if(vehicleType == AIVehicle.VT_AIR && srcHgStation != null && destHgStation != null) { // exchange air?
		...
		if(SquirrelAI.Get().IsInfrastructureMaintenance()) {
			...
			isNearestForPair = srcPlace != null && srcPlace instanceof TownCargo;// ???????????????????
		...
		if(destHgStation == null) {
			if(vehicleType != AIVehicle.VT_WATER) { // CompoundRoute?????NgPlace?????
		...
		SquirrelAI.notBuildableList.AddList(list);
		...
		SquirrelAI.notBuildableList.RemoveList(list);
		if(srcHgStation == null) {
			srcHgStation = SearchSharableStation(src, srcStationFactory.GetStationType(), cargo, false, engineSet.infrastractureType);
			if(srcHgStation != null) {
				isShareSrcStation = true;
...

		{
			...
			if(srcHgStation.stationGroup == null || destHgStation.stationGroup == null) {
				HgLog.Warning("Station was removed."+this); // ??DoInterval??station?Remove????????
				Place.AddNgPathFindPair(src, dest, vehicleType);
				Rollback();
				return null;
			}
			...
			if(SquirrelAI.Get().openttdVersion >= 14 && route.GetVehicleType() == AIVehicle.VT_WATER && route.isTransfer && !isWaitingDestRoute) {
			...
			ClearRollback();
			route.instances.push(route); // ChooseEngine??????????????
			if(!isWaitingProduction && !isWaitingDestRoute) {
				if(route.BuildVehicleFirst() == null && SquirrelAI.Get().IsInfrastructureMaintenance()) {
	...
	function CheckTownTransferCargo(route, station, cargo) {
		...
		if(CargoUtils.IsPaxOrMail(cargo) && station.place instanceof TownCargo && SquirrelAI.Get().CanExtendCoverageAreaInTowns()) {
	...
	function CheckTownTransfer(route, station) {
		foreach(cargo in SquirrelAI.Get().GetPaxMailCargos()) {
	...
	function SearchSharableStation(placeOrGroup, stationType, cargo, isAccepting, infrastractureType=null) {
		...
		if(stationType == AIStation.STATION_DOCK && placeOrGroup instanceof TownCargo && CargoUtils.IsPaxOrMail(placeOrGroup.cargo)) {
			placeOrGroups = []; // water?mail?pax??????
			foreach(eachCargo in SquirrelAI.Get().GetPaxMailCargos()) {
		...
		foreach(eachPlaceOrGroup in placeOrGroups) {
			foreach(station in HgStation.SearchStation(eachPlaceOrGroup, stationType, cargo, isAccepting)) {
				if(stationType == AIStation.STATION_TRUCK_STOP && !isAccepting) {
					if(station.cargo != cargo /*&& station.place != null && station.place.IsProducing()*/) { // Road????cargo?1??station??????????waiting????cargo?vehicle???????
						continue;
					}
				}
				/*
				if(!isAccepting && placeOrGroup instanceof TownCargo) {
					if(station.stationGroup.GetCargoWaiting(cargo) < 300) continue; // town?????????????????
				}*/
...
class InfrastructureCost {
	...
	function GetCostPerDistanceRail(railType) {
		...
		if(distance == 0) {
			...
			if(result==0) return AIRail.GetMaintenanceCostFactor(railType) * 4;//????????
		}
	...
	function GetCostPerAirport(airportType) {
		...
		if(realTotalCosts == 0) {
			return SquirrelAI.GetInflatedMoney(Air.Get().GetAiportTraits(airportType).maintenanceCost);
		}

...
class Air {
	...
	function GetMailSubcargoCapacity(engine) {
		...
		if(real == -1) {
			return AIEngine.GetCapacity(engine) / 8; //???????
		}
	...
	function GetMailSubcargoCapacityReal(engine) {
		...
		local sampleDepot = AirRoute.instances[0].depot; // TODO:???????????????????
		...
		local result = AIVehicle.GetCapacity(vehicle, SquirrelAI.GetMailCargo())
...
class AirRoute extends CommonRoute {
	...
	function GetMaxTotalVehicles() {
		return SquirrelAI.Get().maxAircraft;
	}
	...
	function GetSuitableInfrastractureTypes(src, dest, cargo) { //TODO: ???????????????????
		local result = [];
		foreach(traints in Air.Get().GetAvailableAiportTraits()) {
	...
	function CanCreateNewRoute() {
		...
		if(SquirrelAI.Get().IsInfrastructureMaintenance()) {
			return SquirrelAI.Get().IsRich() /*&& InfrastructureCost.Get().CanExtendAirport()*/;
		} else {
			return true;
		}
	}
	...
	function GetStationDateSpan(self) {
		if((typeof self) == "instance" && self instanceof AirRoute) {	
			local srcUsings = ArrayUtils.Without(srcHgStation.GetUsingRoutes(),this).len()+1; // srcHgStation.GetUsingRoutes???this??????????????
			...
			if(traits.len() >= 1) {
				return traits[traits.len()-1].stationDateSpan + (SquirrelAI.Get().IsEnableVehicleBreakdowns() ? 10 : 0);
			} else {
				return 30;
			}
		}
...
class AirRouteBuilder extends CommonRouteBuilder {
	...
	constructor(dest, src, cargo, options = {}) {
		...
		isNotRemoveStation = SquirrelAI.Get().IsInfrastructureMaintenance() == false;
	...
	function GetUsableAirportTypes(placeOrGroup) {
		local result = [];
		local distanceCorrection = SquirrelAI.Get().isUseAirportNoise ? 1 : 0;
		local limitCost = SquirrelAI.Get().GetUsableMoney() / 4;
		foreach(traits in Air.Get().GetAvailableAiportTraits()) {
			...
			if(placeOrGroup instanceof Place) {
				...
				local town;
				if( SquirrelAI.Get().isUseAirportNoise ) {
...
class AirStation extends HgStation {
	...
	function IsBuildablePreCheck() {
		if(!SquirrelAI.IsBuildable(platformTile)) {
			return false;
		}
		if(!SquirrelAI.IsBuildable(platformTile + AIMap.GetTileIndex(platformNum-1,0))) {
			return false;
		}
		if(!SquirrelAI.IsBuildable(platformTile + AIMap.GetTileIndex(0,platformLength-1))) {
			return false;
		}
		if(!SquirrelAI.IsBuildable(platformTile + AIMap.GetTileIndex(platformNum-1,platformLength-1))) {
			return false;
		}
		return true;
	}
	...
	function BuildStation(joinStation,isTestMode) {
		SquirrelAI.WaitForPrice(AIAirport.GetPrice(airportType));
	...
	function CanShareByMultiRoute(infrastractureType = null, cargo = null) {
		...
		foreach(route in usingRoutes) {
			...
			if(arrivesPerYear > 365 / traits.stationDateSpan / 2) { // ??????????????????????
				return false;
			}
		}
		return true;

...
class Path {
			...
			while(path != null) {
				if(prev != null) {
					if (AIMap.DistanceManhattan(prev, path.GetTile()) > 1) {
						...
						/*if(vehicleType == AIVehicle.VT_WATER && AIMarine.IsCanalTile(prev)) { LOST????
	...
	function BuildDoubleDepotOn(prev, cur, next) {
		...
		if(rl != 0) {
			RailBuilder.ChangeTunnel(cur-2+dir*rl,cur+2+dir*rl); // ????????????????
			local depots = HgTile(cur).BuildDoubleDepot(depot1, depot2, prev, next);
			if(depots != null) {
				return {mainTiles=[prev,cur,next],depots=depots};
			}
	...
	function GetSlopes(length, endTile = null) {
		...
		for(;endPath != null && (endPrev==null || endPrev!=endTile); 
			...
			if(endPrev != null) {
				...
				endHeight += AIBridge.IsBridgeTile(endPrev) ? 1 : 0; // TODO: ????
			}
...
class BuildedPath {
	...
	static function AddTiles(tiles,obj) { //hgstation???????
	...
	function Remove(removeRails = true, doInterval = false) {
		if(removeRails) {
			...
			//path.Reverse().RemoveRails(route, false/*isTest*/, doInterval); //Reverse()?????????????
		}
		BuildedPath.instances.rawdelete(this);
		//BuildedPath.RemoveTiles(path.GetTiles()); ???????????????????????????????????????
	}
...
class RailBuilder extends Construction {
	...
	static function BuildRailForce(p1,p2,p3) {
		...
		if(AICompany.IsMine(AITile.GetOwner(p2))) { // ??????????????
			return false;
		}
	...
	static function GetBridgeIDSafe(tile, other = null) {
		try {
			return AIBridge.GetBridgeID(tile);
		} catch(e) {
			HgLog.Warning("AIBridge.GetBridgeID unavailable:"+HgTile(tile)+" "+e);
		}
		if(other == null && AIBridge.IsBridgeTile(tile)) {
			other = AIBridge.GetOtherBridgeEnd(tile);
		}
		if(other == null) {
			return null;
		}
		local bridge_list = AIBridgeList_Length(AIMap.DistanceManhattan(tile, other) + 1);
	...
	static function RemoveRailTracksAll(tile) {
		...
		foreach(railTrack in RailBuilder.RailTracks) {
			if(tracks & railTrack) {
				AIRail.RemoveRailTrack(tile,railTrack); // ?????????????
			}
		}
		return true;
	}
	...
	function DoBuild() {
		...
		local prevprevprev = null;
		local signalCount = 7;
		local checkTownBus = false; // ?????? SquirrelAI.Get().IsRich();
		while (path != null) {
			if(prevprev != null) {
			...
			if (prev != null && AIMap.DistanceManhattan(prev, path.GetTile()) > 1) {
				...
				if(underground != null) {
					SquirrelAI.WaitForMoney(50000,0,"BuildUnderground");
					...
					if(AITunnel.IsTunnelTile(prev) != path.GetTile() || !AICompany.IsMine(AITile.GetOwner(prev))) {
						SquirrelAI.WaitForMoney(50000,0,"BuildTunnel");
	...
	//								AIController.Break("");
				...
				if(!AIRail.AreTilesConnected(prevprev,prev,path.GetTile()) || !AICompany.IsMine(AITile.GetOwner(prev))) {
					if(checkTownBus) TownBus.Check(prev); // Refactor??demolish???????
					...
					local isGoalOrStart = (prevprevprev == null && AITile.HasTransportType(prevprev,AITile.TRANSPORT_RAIL)) 
						|| (path.GetParent()==null && AITile.HasTransportType(path.GetTile(),AITile.TRANSPORT_RAIL)); //HasTransportType??????????????????????
						/*|| path.GetParent().GetParent()==null  ???ChangeBridge???????????isReverse???????? */;
	...
	function DemolishTile( tile ) {
		if(	AIRail.IsRailStationTile( tile ) ) {
			AIRail.RemoveRailStationTileRectangle (tile, tile, false);// join????????????????????????
		} else {
	...
	function RetryToBuild(path,prev) {
		if(retryCount >= 4) {
			HgLog.Warning("RetryToBuild retry count reached limit.");
			return false;
		}
		...
		local retryIgnoreTiles = [];
		if(ignoreTiles != null) {
			retryIgnoreTiles.extend(ignoreTiles);
		}
		retryIgnoreTiles.push(prev);
		HgLog.Info( "RetryToBuild start prev:"+HgTile(prev)+"-"+HgTile(path.GetTile())
			+" newGoals:"+HgTile.GetTilesString(goalsArray[0])+" start:"+HgTile(startPath.tile)+"-"+HgTile(startPath.GetLastTile())+" isReverse:"+isReverse+" retryCount:"+retryCount );
		local railBuilder = RailToAnyRailBuilder(isReverse?startPath.Reverse():startPath, 
			goalsArray, retryIgnoreTiles, !isReverse, 80, eventPoller, pathFinder);
		railBuilder.retryCount = retryCount + 1;
		railBuilder.isNoSignal = isNoSignal;
		local result = railBuilder.Build(); // TODO: Rollback??
		built.Remove(!result); // ?????????????????????
		if(result) {
			local newTiles = railBuilder.pathSrcToDest.Reverse().GetTiles();
			local orgTiles = startPath.SubPathEnd(newTiles[0]).GetTiles();
			BuildedPath( startPath.SubPathIndex(startPath.GetIndexOf(newTiles[0])-1) ).Remove(true); // ??????????
	...
	function FlattenRails(buildedPath) {
		...
		for(; path != null; prev = path.GetTile(), path = path.GetParent()) {
			...
			if(SquirrelAI.GetUsableMoney() < SquirrelAI.GetInflatedMoney(300000)) {
				break;
			}
			SquirrelAI.WaitForMoney(50000,0,"FlattenRails");
	...
	function RebuildForHomeward(buildedPath) {
		...
		for(; path != null; prev = path.GetTile(), path = path.GetParent()) {
			...
			SquirrelAI.WaitForMoney(50000,0,"ForceLevelBound(RebuildForHomeward)");
	...
	function BuildUnderground(path,prev,prev2,prev3,underground) {
		...
		if (AITunnel.GetOtherTunnelEnd(A2) == B2) {
			SquirrelAI.WaitForMoney(50000,0,"BuildTunnel(BuildUnderground)");
	...
	function RaiseTileIfNeeded(prevpath,prevprev) {
		...
		if(AIBridge.IsBridgeTile (prevprev)) {	
			...
			while(true) {
				if(AIMap.DistanceManhattan(t0,t1)>1 || RailPathFinder._IsUnderBridge(t1)) { // ??????????
	...
	static function BuildBridgeSignals(signals) {
		foreach(signal in signals) {
			// ????????????????????????????????????????????10??
			SquirrelAI.Get().PostPending(10, SignalBuilder(signal[0], signal[1], AIRail.SIGNALTYPE_PBS_ONEWAY) );
		}
	}
	
	
	static function ChangeTunnel(start,end) { // depot???????????5tile??
		...
		SquirrelAI.WaitForMoney(20000,0,"ChangeTunnel");
		local currentRailType = AIRail.GetCurrentRailType();
...
class RailPathBuilder extends Construction {
	...
	static function GetStartArray(path) {
		...
		for(;path != null; prev3 = prev2, prev2 = prev, prev = path.GetTile(), path = path.GetParent()) {
			...
			if(d1==1 && d2==1 && !AIRail.AreTilesConnected(next,cur,prev)) { /*double depots?????????*/
				continue;
			}
			if(d2==1 && d3==1 && !AIRail.AreTilesConnected(cur,prev,prev2)) { /*double depots?????????*/
				continue;
			}
	...
	function OnPathFindingInterval() {
		if(eventPoller != null) {
			try {
				...
				// Optional external poller may be missing this callback; ignore safely.
			}
		}
	...
	function DoBuild() {
		...
		isNotCheckRevPath = true; // RailBuilder??pathFinder2?????????
...
class RailToAnyRailBuilder extends RailBuilder {
	...
	static function GetStartArray(path) {
		...
		path.IterateRailroadPoints(function(prev3,prevprev,prev,fork,next):(result) {
			if((SquirrelAI.IsBuildable(fork) || ( AICompany.IsMine(AITile.GetOwner(fork)) && RailPathFinder.CanChangeBridgeStatic(fork,false)))
...
class RailRemover extends Construction {
	...
	function _DoBuild() {
		// ??AddRollback?????????=>AddRollback???save?????Rollback???????
		// ???Rollback??????????????
		...
		for(local i=1; i<size; i++) {
			...
			if(AIBridge.IsBridgeTile(array_[i]) && i+1<size
				...
				if(rollbackIfFailed) {
					local bridgeId = RailBuilder.GetBridgeIDSafe(array_[i], array_[i+1]);
					if(bridgeId == null) {
						HgLog.Warning("GetBridgeIDSafe failed:"
							+HgTile(array_[i])+"-"+HgTile(array_[i+1]));
						return false;
					}

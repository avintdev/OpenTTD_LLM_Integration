...
/*			
	...
	function ClearCargoInfos() { // SquirrelAI.GetMaxCargoPlaces()??????
	...
	function CalculateProductionInfos(cargo) {
		...
		foreach(place in Place.GetCargoPlaces(cargo,true)) {
			...
			if(place.GetLastMonthTransportedPercentage(cargo) == 0) {
				pieceInfo.sum += place.GetLastMonthProduction(cargo);
				pieceInfo.count ++;
				pieceInfo.places.push(place);
			} else {
	...
	function SetDirtyArround(location, cargo) {
		if(cargoProductionInfos == null) { // cargoProductionInfos???????????
			return;
		}
	...
	function GetPlacesInIndexes(cargo, isProducing, indexes) {
		...
		foreach(index in indexes) {
			result.extend(pieceInfos[index].places);
			result.extend(pieceInfos[index].usedPlaces);
		}
		return result;
	}
...
class DistancePlaces {
	...
	constructor(places, orgTile) {
		...
		foreach(place in places) {
			...
			local distanceIndex = SquirrelAI.distanceSampleIndex[min(distanceSample, SquirrelAI.distanceSampleIndex.len()-1)];
			local dplaces;
...
class CargoDistancePlaces {
	...
	function GetPlaces(distanceIndex) {
		if(!indexPlaces.rawin(distanceIndex)) {
			local range = SquirrelAI.GetEstimateRange(SquirrelAI.distanceEstimateSamples, distanceIndex);
			if(range[0]==null) range[0] = 0;
			if(range[1]==null) range[1] = SquirrelAI.distanceEstimateSamples[distanceIndex];
...
class Place {
	...
	static function LoadStatics(data) {
		...
		if(data.rawin("ngCandidatePlaces")) {
			...
			foreach(k,v in Place.ngCandidatePlaces) {
				/*local s = k.find(":");
				HgLog.Warning("ngCandidatePlaces:"+HgTile(k.slice(0,s).tointeger())+" "+AICargo.GetName(k.slice(s+1).tointeger())+" "+v);*/
			}
	...
	static function IsNgPlace(facility, cargo, vehicleType) {
		...
		if(facility instanceof HgIndustry && AIIndustry.GetAmountOfStationsAround(facility.industry) >= 1) {
			if(SquirrelAI.Get().IsAvoidSecondaryIndustryStealing()) {
				...
				/*
				if(AIIndustry.GetAmountOfStationsAround(facility.industry) >= 1 && facility.GetRoutes().len() == 0) { //TODO: ??????????????????station????????
					HgLog.Info("Detect SecondaryIndustryStealing:"+facility.GetName());
					return true;
				}*/
			}
...
/*		???????? if(facility instanceof HgIndustry && facility.GetRoutes().len() == 0) {
	...
	static function AddNgPathFindPair(from, to, vehicleType, limitDay = null) {
		...
		HgLog.Info("AddNgPathFindPair from:"+from+" to:"+to+" "+Route.Class(vehicleType).GetLabel()+" limitDay:"+limitDay);
	...
	static function GetCargoProducing( cargo ) {
		if(SquirrelAI.Get().IsFreightOnly() && CargoUtils.IsPaxOrMail(cargo)) {
			return HgArray([]);
		} else if(SquirrelAI.Get().IsPaxMailOnly() &&!CargoUtils.IsPaxOrMail(cargo)) {
			return HgArray([]);
		}
		local result = [];
		foreach(industry,v in AIIndustryList_CargoProducing(cargo)) {
		...
		//HgLog.Warning("GetCargoProducing:"+AICargo.GetName(cargo)+" "+result.len());
	...
	static function GetCargoAccepting(cargo) {
		...
		result.array.extend(HgArray.AIListKey(AIIndustryList_CargoAccepting(cargo)).Map(function(a) {
			...
			return place.IsCargoAccepted(cargo); //CAS_TEMP_REFUSED?????
		}).array);
	...
	static function IsProducedByTown(cargo) {
		return CargoUtils.IsPaxOrMail(cargo); //TODO: ???????
	}
	...
	static function SearchAdditionalAcceptingPlaces(cargos, srcTiles ,lastAcceptingTile, maxDistance, minPopulation, vehicleType=AIVehicle.VT_RAIL) {
		...
		hgArray = HgArray(places).Filter(function(place) : (cargos,minPopulation) {
			...
			if(t.distance > 0) {
				...
				t.score <- score * 100 / srcTilesScores.len() / t.distance; //????100?????cargo???????????
			} else {
				t.score <- 0;
			}
			return t;
	...
	function CanUseNewRoute(cargo, vehicleType) {
		if(vehicleType == AIVehicle.VT_AIR && !SquirrelAI.Get().IsInfrastructureMaintenance()) {
			// ?????????????????????
			return true;
		}
		if(this instanceof TownCargo) {
			//return true;
			local maxStationGroups = GetPopulation() / 10000 + 1; // TODO: spread?????????????
			local count = 0;
		...
		//HgLog.Warning( "canUsePlaceOnWater:"+SquirrelAI.Get().canUsePlaceOnWater+" "+this.IsBuiltOnWater()+" "+this.IsNearLand(cargo) );
		if(!SquirrelAI.Get().canUsePlaceOnWater && this.IsBuiltOnWater() && !this.IsNearLand(cargo)) {
			return false;
		}
		if(SquirrelAI.Get().firs && CargoUtils.IsSupplyCargo(cargo)) { // supply cargo
	...
	/*
		...
		if(Place.placeCargoUsingRoutes.rawin(key)) {
			...
			foreach(routeId,_ in usingRoutes) {
				if(Route.allRoutes.rawin(routeId)) { // remove?????????
...
/*
	...
	function GetFutureExpectedProduction(cargo, vehicleType, isMine = false) {
		local rate = 100;
		if((CargoUtils.IsPaxOrMail(cargo) || (!SquirrelAI.Get().ecs && !SquirrelAI.Get().firs)) && IsIncreasable()) {
			rate = 150;
		}
		return GetExpectedProduction(cargo,vehicleType,isMine) * rate / 100;
		
		/*
		if(SquirrelAI.Get().buildingTimeBase && !SquirrelAI.Get().ecs) {
			if(IsIncreasable() && vehicleType == AIVehicle.VT_RAIL) {
				return GetExpectedProduction(cargo,vehicleType,isMine) * 3; // ????????????
			}
		}
	...
	function GetProductionArroundCount(cargo, vehicleType) {
		if(!SquirrelAI.Get().roiBase && !SquirrelAI.Get().ecs && IsProcessing() 
	...
	function _GetSupportEstimate(isMin) {
		...
		foreach(acceptingCargo in GetAccepting().GetCargos()) {
			...
			foreach(vehicleType,_ in vehicleTypes) {
				...
				local connected = false; // ??place???cargo
	...
	function _GetExpectedProductionAll(cargo, vehicleType, isMin) {
		...
		local placeProduction = PlaceProduction.Get();
		if( !SquirrelAI.Get().ecs && !SquirrelAI.Get().firs ) {
			if( SquirrelAI.Get().buildingTimeBase && IsProcessing()) { //support route??????buildingTimeBase??
//				production += GetSupportRouteEstimate(1000).production * 2 / 3;
		...
		if(SquirrelAI.Get().firs && !SquirrelAI.Get().roiBase) {
			/*if(!preEstimate) {
				if(IsRaw()) { 
					// ????????????????3????
		...
		if(SquirrelAI.Get().ecs /*GetUsableMoney() >= SquirrelAI.Get().GetInflatedMoney(2000000)*/ && IsRaw() && production >= 1) {
	...
	function AdjustUsing(production,cargo,isMine) {
		...
		if(count == 0 && (!isTownCargo || (isTownCargo && !TownBus.Exists(town,cargo)))) { // ???1???????????
			return 70 * production / (GetLastMonthTransportedPercentage(cargo) + 70);
		} else if(count == 1 && isMine) {
			return production / (otherCompanies + 1);
		} else { 
			return (production  / (otherCompanies + 1)) / max(1,count + (isMine?0:1));
		}
	}
	...
	function IsEcsHardNewRouteDest(cargo) {
		if(SquirrelAI.Get().ecs && this instanceof HgIndustry && this.GetCargos().len() >= 2) { // ecs??????????industry???????????????????
	...
	function FindCoast(cargo) {		
		...
		if(IsBuiltOnWater()) {
			local cur = GetLocation(); // ????????IsBuiltOnWater???(firs)
	...
	function GetAllowedAirportLevel(airportType/*??????aiportType*/, cargo) {
	...
	function IsMeetExceptProcessing() { // raw,Wharf?????????
		local cargos = GetAccepting().GetCargos();
		local accept = 0;
		...
		if(IsRaw()) {
			return accept >= 27;// 80/3=27/?
		} else {
			return accept >= 213;// 640/3=213/?
		}
	}
				...
				if( cargoIncomes.rawin(producingCargo) ) {
					...
					if(usingRoute instanceof TrainRoute) {
						if(usingRoute.returnRoute != null && !usingRoute.returnRoute.NeedsAdditionalProducingCargo(producingCargo)) {
							result += cargoIncome * rate / 100; // ?????TODO: ???bidirection?
						}
					}
				}
			}
	...
	function GetAdditionalRouteIncome(cargo) {
		...
		foreach(route in GetRoutesUsingSource()) {
			if(CargoUtils.IsPaxOrMail(route.cargo)) { //PaxMail?????(HOTEL)
				return 0;
			}
			local engineSet = route.GetLatestEngineSet();
			if(engineSet != null) {
				if(isProcessing) {
					// processing????accept5??2?????????1/5??2/5???(???1/5???)
					result += engineSet.routeIncome / cargos.len();
				} else {
					result += engineSet.routeIncome * 2; // raw,Wharf?3????
				}
			}
		}
		return result;
...
class HgIndustry extends Place {
	...
	function _IsNearWater(cargo) {
		...
		SquirrelAI.Get().pendingCoastTiles.push(coastTileList.Begin());
		return true;
	}
	...
	function _GetCargos() {
		if(isProducing) {
			...
			if(traits == "OIL_,PASS,/") { // ????
				return SquirrelAI.Get().GetPaxMailCargos();
			}
	...
	function _IsIncreasable() {
		if(SquirrelAI.Get().ecs || SquirrelAI.Get().yeti) {
			return true;
		}
...
class TownCargo extends Place {
	...
	function GetCargoTileList(coverageRadius,cargo) {
		...
		if(TownBus.CanUse(cargo) || SquirrelAI.Get().CanExtendCoverageAreaInTowns()) {
			maxRadius += coverageRadius; //pax?bus????????????
		}
		if(IsProducing()) {
	...
	function GetSpreadPopulation(spread) {
		...
		if(!TownCargo.spreadPopulation.rawin(spread)) {
			TownCargo.spreadPopulation.rawset(spread,_GetSpreadPopulation(spread-5/*??????? TODO:Road???????????*/));
		}
		return TownCargo.spreadPopulation.rawget(spread);
	}
	
	function _GetSpreadPopulation(spread) {
		if(spread <= 3) return 700;
		if(spread <= 4) return 1200;
		if(spread <= 5) return 1800;
		if(spread <= 6) return 2700;
		if(spread <= 7) return 3700;
		if(spread <= 8) return 5000;
		if(spread <= 9) return 6500;
		if(spread <= 10) return 8200;
		if(spread <= 11) return 10200;
		if(spread <= 12) return 12400;
		if(spread <= 13) return 14900;
		if(spread <= 14) return 17700;
		if(spread <= 15) return 20800;
		if(spread <= 16) return 24200;
		if(spread <= 17) return 27900;
		if(spread <= 18) return 32000;
		return 36300;
	}
	...
	function GetTiles(coverageRadius,cargo) {
		...
		return null;
/*		
		result.Valuate(SquirrelAI.IsBuildable);
		result.KeepValue(1);
	...
	function GetNotUsedProductionMap(exceptPlatformTiles) {
		...
		foreach(tile in Rectangle.Center(HgTile(GetLocation()),GetRadius()).GetTiles()) {
			...
			if(!exceptPlatformMap.rawin(tile)) { // ??????????????platform?????
	...
	function GetExpectedUsingDistantJoinStations() {
		local d = max(5,SquirrelAI.Get().maxStationSpread);
		return 200 + d * d * 2;
	}


	function GetExpectedProductionAll(cargo, vehicleType, isMine = false, preEstimate = false) {
		local result = _GetExpectedProductionAll(cargo, vehicleType, isMine, preEstimate);
		if(!CanGrowth()) result = result * 2 / 3;
		return result;
	}

	function _GetExpectedProductionAll(cargo, vehicleType, isMine = false, preEstimate = false) {
		local production;
		/*if(isCurrent) {
		...
		if(SquirrelAI.Get().IsDistantJoinStations() && !SquirrelAI.Get().IsAvoidExtendCoverageAreaInTowns()) {
			...
			local d = SquirrelAI.Get().maxStationSpread;
			local population = AITown.GetPopulation(town);
			if(!TownBus.CanUse(cargo) && population!=0) production = production * min(population, GetSpreadPopulation(d)) / population;
			local additinalTownNum = max(0,(Place.GetTownDensity() * d * d).tointeger() - 1);
			//HgLog.Info("TownCargo.GetExpectedProductionAll:"+production+" "+AICargo.GetName(cargo)+" "+AITown.GetName(town)+" additinalTownNum:"+additinalTownNum);
			production += additinalTownNum * Place.GetAverageTownPopulation() / (cargo == SquirrelAI.Get().GetPassengerCargo() ? 7 : 20);
			return min( production , cargo == SquirrelAI.Get().GetPassengerCargo()
			...
			production = production * 2 / 3; // ??????????
			if(vehicleType == AIVehicle.VT_ROAD) {
				return min( production , cargo == SquirrelAI.Get().GetPassengerCargo() ? 200 : 80 );
			} else {
				return min( production , cargo == SquirrelAI.Get().GetPassengerCargo() ? 550 : 220);
			}
		} else {
			local minValue = cargo == SquirrelAI.Get().GetPassengerCargo() ? 200 : 80;
			if(vehicleType == AIVehicle.VT_ROAD) {
				minValue /= 2;
	...
	function GetLastMonthProduction(cargo) {
		...
		if(r > AITown.GetPopulation(town)) { // ???????????
			if(cargo == SquirrelAI.Get().GetPassengerCargo()) {
				r = AITown.GetPopulation(town) / 8;
			} else if(cargo == SquirrelAI.Get().GetMailCargo()) {
...
/*		
	...
	function GetProducingCargos() {
		...
		local mail = SquirrelAI.Get().GetMailCargo();
	...
	function GetAllowedNoise(airportType/*HgIndustry???*/) {
		return AITown.GetAllowedNoise(town) + (SquirrelAI.Get().isUseAirportNoise ? 1 : 0)/*?????????????????????????*/;
	}

...
class RailPathFinder
{
	...
	revOkTiles = null;
	orgTile = null;
	trainDirection = null; // ?????? 0:goal?? 1:start?? 2:???
	...
	function InitializeParameters() {
		...
		_cost_tunnel_per_tile = 0;//100;//50 // ????????????????????0?
		...
		_bottom_ex_bridge_length = 7;//platformLength == null ? 7 : max( 7, platformLength ); ??????????????????????????
		...
		} else if(!SquirrelAI.Get().HasIncome(100000)) {
	...
	function InitializePath(sources, goals, ignored_tiles = [], reversePath = null) {
		local testMode = AITestMode(); // _Cost???????????
	...
	function __Cost(self, path, new_tile, new_direction, mode) {
		...
		if(self.isOutward) { // ??????
			...
			if(distance > 1) {
				...
				if(!SquirrelAI.IsBuildable(t[1] + revDir) || !SquirrelAI.IsBuildable(t[0] + revDir) 
						|| HgTile.GetBoundMaxHeight(t[0] + revDir, t[1] + revDir) == 0
						|| (turn && ( 
							(dirs[1]==revDir && AITile.IsCoastTile(t[1]) && HgTile(t[1]).GetMaxHeightCount()<3) // ???????
	...
	//				HgLog.Info("c_node:"+HgTile(c_node)+" node:"+HgTile(node)+" end:"+HgTile(end));
					if(offset == 1 && c_node - end >= offsets[0]) { // offset?x???end?Y????????????????????
	...
	//		tiles.push([next_tile, 1]);
			...
			foreach (offset in offsets) {
				...
				if (par == null 
						...
						|| (underBridge && self._IsSlopedRail(par_tile, cur_node, next_tile)) // ???????????????
						|| (goal2 == next_tile 
	...
	function _IsBuildableSea(tile, next_tile) {
		return ( AITile.IsSeaTile(tile) || (AITile.IsCoastTile(tile) && AITile.IsBuildable(tile) && AITile.GetMaxHeight(tile) <= 1/*steep slope????*/) )
	...
	function _GetTunnelsBridges(par, last_node, cur_node) {
		...
		if(par.GetParent()!=null) {
			...
			if(dir == last_node - last_node2 
					...
					&& !_CanBuildBridge(cur_node,_GetNextBridgeEnd(cur_node,last_node),last_node)/*???????????????(??)*/) {
		...
		if((!AITile.IsBuildable(next) && !SquirrelAI.IsPurchasedLand(next)) || level > AITile.GetMaxHeight(next)
				|| (isOutward && !AITile.IsBuildable(next + revDir))) {
		...
		if(AITile.IsCoastTile(cur_node)) { //????????????????????????
			return tiles;
		}

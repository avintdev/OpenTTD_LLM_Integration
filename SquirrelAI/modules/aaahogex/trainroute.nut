...
class TrainRoute extends Route {
	...
	static function SaveStatics(data) {
		...
		data.removedTrainRoute <- arr; //??????IsInfrastractureMaintenance:true??????????
	...
	static function LoadFrom(t) {
		...
		foreach(stationId in t.destHgStations) {
			if(stationId != null) {
				if(HgStation.worldInstances.rawin(stationId)) { // station?? => destHgStations????????????
	...
	static function GetIdealDistance(cargo) {
		...
		foreach(distanceIndex, distance in SquirrelAI.distanceEstimateSamples) {
			...
			//local value = estimate.routeIncome; //roi?????buildingTimeBase???????????????
			local value = SquirrelAI.Get().buildingTimeBase ? estimate.routeIncome : estimate.value; // ???????buildingTime?????????????
	...
	function GetEstimator(self) {
		...
		result.skipWagonNum = 5; //SquirrelAI.Get().roiBase ? 3 : 5;
	...
	startDate = null;
	subCargos = null; // ??????cargo(mainCargo??)
	destHgStations = null;
	srcDepot = null;
	destDepot = null;
	branchLines = null;
	...
	function GetMaxTotalVehicles() {
		return SquirrelAI.Get().maxTrains;
	}
...
/*
	...
	function GetBuildingTime(distance) {
		//return distance / 2 + 100; // TODO expectedproduction???????????????
//		return distance * 3 / 2 + 2100;
	...
	/*
		...
		if(latestEngineSet != null && engineSetsCache != null) {
			if(engineSetsDate + latestEngineSet.days < AIDate.GetCurrentDate()) { // ?????????1?????????
...
/*
		...
		foreach(index, cargo in cargos) {
			...
			if(this.cargo == cargo) {
				production = max(50, production); // ??0????????????????????BuildFirstTrain???????????
			}
	...
	function GetRoundedProduction(production) {
		...
		local index = SquirrelAI.Get().GetEstimateProductionIndex(production);
		return SquirrelAI.Get().productionEstimateSamples[index];
	}
	...
	function GetEngineSets(isAll=false, additionalDistance=null) {
		...
		if(!isAll && additionalDistance==null && engineSetsCache != null && engineSetsCache.len() >= 1) {
			if(TrainRoute.instances.len()<=1 && SquirrelAI.Get().roiBase) {
			...
			if(latestEngineSet!=null && AIDate.GetCurrentDate() < engineSetsDate + max(minDesignSpan,latestEngineSet.cruiseDays * 6)) {
				return engineSetsCache; // ??1??3???????
			}
			if(latestEngineSet!=null && AIDate.GetCurrentDate() < engineSetsDate + 10 * 365 ) {
				if(productionChackDate==null || productionChackDate + 365 < AIDate.GetCurrentDate()) { // 1??????????
					...
					if(oldCargoProd.len() == cargoProd.len()) {
						...
						foreach(cargo,newProd in cargoProd) {
							...
							if(newProd > oldProd && srcHgStation.GetCargoWaiting(cargo) > 1000) { // ????10??????cargo?????????????
								ok = false;
								break;
							}
						}
	...
	function _BuildFirstTrain() {
		saveData.latestEngineVehicle = latestEngineVehicle = BuildTrain(); //TODO ??????????????????order??????????????????????????order?????????????????
	...
	function BuildNewTrain(depotTile=null) {
		...
		if(latestVehicle == null) {
			...
			BuildOrder(newTrain); // ????????????????????ChangeDestination?????????????Sell?????????
...
//		}
	...
	function CloneAndStartTrain(isDest, latestEngineVehicle) {
		...
		if(!HasVehicleEngineSet(latestEngineVehicle, engineSet)) {
			...
			foreach(cargo,capacity in engineSet.cargoCapacity) {
				if(!oldCargoCapacity.rawin(cargo)) { // TODO: cargo????????
			...
			if(newCargo != null){
				foreach(route in destHgStation.stationGroup.GetUsingRoutesAsSource()) {
					route.NotifyAddTransfer(newCargo); // TODO: ??cargo
				}
				if(IsBiDirectional()) {
	...
	function CloneTrain(isDest, latestVehicle) {
		...
		for(local need=20000;; need+= 10000) {
			local r = SquirrelAI.WaitForMoney(need);
	...
	function BuildTrain(mode = 0, depotTile = null) {
		...
		foreach(engineSet in GetEngineSets(isAll)) {
			...
			for(local i=0; i<numEngineVehicle; i++) {
				...
				if(r == null) {
					return null; // ERR_VEHICLE_TOO_MANY???
				} else if(!r) {
					success = false;
					break;
				}
			...
			if(!success) {
				// AddUnsuitableEngineWagon(trainEngine, engineSet.wagonEngineInfos[0].engine); wagon??????????????
				continue;
			}
			...
			foreach(wagonEngineInfo in engineSet.wagonEngineInfos) {
				for(local i=0; i<wagonEngineInfo.numWagon; i++) {
					...
					if(!AIVehicle.IsValidVehicle(wagon))  {
						// AddUnsuitableEngineWagon(trainEngine, wagonEngineInfo.engine); wagon??????????????
					...
					if(realLength != trainInfo.length) { // ???????
			...
			if(engineSetsCache != null && engineSetsCache.len() >= 1) {
				engineSetsCache[0] = engineSet; // ChooseEngineSet()????????engineSet????????
			}
			if(returnRoute != null) {
	...
	function AddReturnTransferOrder(vehicle, transferSrcStation, destStation) {
		local execMode = AIExecMode();
		// destStation?LOAD
		// PAX??????? AIOrder.SetOrderFlags( latestEngineVehicle, AIOrder.GetOrderCount(latestEngineVehicle)-1, AIOrder.OF_NON_STOP_INTERMEDIATE + AIOrder.OF_UNLOAD);
		// YARD
	...
	function AddDestination(destHgStation) {
		...
		destHgStation.AddUsingRoute(this); // ChangeDestination??????????????
	...
	function ChangeDestination(destHgStation, checkAcceleration = true) {
		if(returnRoute != null) { // ???????return route?????????????
		...
		//oldDestHgStation.RemoveOnlyPlatform();// ??????????????????????????????????????(SendDepot???????????????????????)
		...
		if(latestEngineVehicle != null) {
			...
			local failed = false;
			if(AIOrder.GetOrderCount (latestEngineVehicle) >= 4) { // return route??????change??????????????
...
/*				local tracks = AIRail.GetRailTracks(cur);
	...
	function ConvertRailType(railType,force=false) {
		...
		foreach(t in tiles) {
			if(AIRail.IsLevelCrossingTile(t)) { // ????RailType???????????????????????
		...
		while(tileTable.len() >= 1) {
			foreach(tile,_ in tileTable) {
				...
				// destHgStations????????????????????????????????
				/*???????????if(!AIRail.IsRailTile(tile) && !AIRail.IsRailDepotTile(tile) && !AIBridge.IsBridgeTile(tile) && !AITunnel.IsTunnelTile(tile)) {
					continue;
				}*/
		...
		convertedList.Valuate(AIRail.GetRailType); // ???????AIRail.ConvertRailType????????????????
		...
		if(updateRailDepot != null) {
			foreach(t in tiles) {
				if(AIBridge.IsBridgeTile(t)) {
					...
					local currentBridge = RailBuilder.GetBridgeIDSafe(t, other);
					if(currentBridge != null && latestBridge != currentBridge) {
...
/*
	...
	function OnVehicleWaitingInDepot(engineVehicle) {
		...
		if(updateRailDepot != null) {
			...
			if(isRemoved || latestEngineVehicle != engineVehicle) { //reopen????latestEngineVehicle????
				SellVehicle(engineVehicle);
			}
		} else {
			SellVehicle(engineVehicle);
	...
	function CheckTrains() {
		...
		if( AIBase.RandRange(100) < 10 && CargoUtils.IsPaxOrMail(cargo)) { // ???????????????
			foreach(townCargo in SquirrelAI.Get().GetPaxMailCargos()) {
				if(!HasCargo(townCargo)) continue;
				if(!IsOverflow(townCargo,false)) { //NeedsAdditionalProducing?town????production???????????????false???????
		...
		if(!IsBuilding() && lastChangeDestDate != null && lastChangeDestDate + 120 < AIDate.GetCurrentDate() && !IsChangeDestination()) {
			...
			foreach(i,station in destHgStations) {
				if(station != destHgStation && station != destHgStations[destHgStations.len()-1]) {
					if(SquirrelAI.Get().ecs && station.place != null && !(station.place instanceof TownCargo)) {
						continue;
					}
					/*
		...
		if(!SquirrelAI.HasIncome(20000) || !ExistsMainRouteExceptSelf()) {
		...
		if(AIBase.RandRange(100) < 10 && SquirrelAI.Get().IsEnableVehicleBreakdowns()
...
/*			local destRoute = GetDestRoute();
			...
			for(acceptableStationIndex=destHgStations.len()-1; acceptableStationIndex>=0 ;acceptableStationIndex--) {
				if(destHgStations[acceptableStationIndex].IsRemoved()) { // dest????????????????Remove????????
					continue;
				}
			...
			if(isClosed) {
				...
				if(acceptableStationIndex == -1 && returnRoute != null) {
					if(destHgStations[destHgStations.len()-1].place != null && destHgStations[destHgStations.len()-1].place.IsClosed()) {
						...
						Remove(); //TODO ????????Close????Remove???????Remove??????station.place????????????????????????Remove
					} else {
						//if(!SquirrelAI.Get().ecs) { ?????????? //???????????????????????????(ECS)
							Close();
						//}
					}
				}
...
class TrainReturnRoute extends Route {
	...
	function _get(idx) {
		switch (idx) {
			...
			case "lastDestClosedDate": // TODO: return route?dest close??????????????????????????src?cargo???????????????route???
				return null;
			default:
...
class TrainRouteBuilder extends RouteBuilder {
	...
	function DoBuild() {
		...
		//SearchAndBuildAdditionalDest(route); place??????????????????
		return route;
	}

	// ???: TrainRoute ???null, ??????false???
	function BuildRoute(dest, src, cargo, options) {
		...
		if(engineSet==null) {
			...
			local trainEstimator = TrainEstimator();
			trainEstimator.cargo = cargo;
			trainEstimator.isSingleOrNot = isSingleOrNot; //src???????????????????????????????????
		...
		local useSingle = engineSet.isSingle; //SquirrelAI.Get().GetUsableMoney() < SquirrelAI.Get().GetInflatedMoney(100000) && !SquirrelAI.Get().HasIncome(20000);
		if(useSingle) {
		...
		if(destHgStation == null) {
			...
			if(canChangeDest && SquirrelAI.Get().roiBase && SquirrelAI.Get().GetQuarterlyIncome() > SquirrelAI.Get().GetInflatedMoney(50000)) {
				estimatedPlatformLength *= 2; //roiBase?????????????
			}
		...
		local SquirrelAI = SquirrelAI.Get();
		...
		SquirrelAI.pathfindings.rawset(pathfinding,0);
		...
		local adjustedPathFindLimit = !SquirrelAI.HasIncome(10000) && !SquirrelAI.IsRich() ? SquirrelAI.pathFindLimit * 3 : SquirrelAI.pathFindLimit;
		...
		railBuilder.pathBuildParams = {
			...
			distance = distance
			isBiDirectional = isBiDirectional || !canChangeDest // ????????????
		...
		if(useSingle) {
			...
			if(SquirrelAI.IsEnableVehicleBreakdowns()) {
				if(useSimpleStation) {
					//railBuilder.isBuildSingleDepotDestToSrc = true;
				}
		...
		SquirrelAI.pathfindings.rawdelete(pathfinding);
		...
		if(!isSuccess || srcDepot == null) {
			...
			HgStation.AddNgStationTile(srcHgStation); // station?????????????????
		...
		if(srcHgStation.stationGroup == null || destHgStation.stationGroup == null) {
			HgLog.Warning("TrainRoute: station was removed."+explain); // ?????????????????????????????
			Rollback();
			return null;
		}
...
class TrainRouteExtendBuilder extends RouteModificatin {
	...
	function DoBuild() {
		...
		railBuilder.Initialize(
			...
			additionalHgStation, SquirrelAI.Get().pathFindLimit, SquirrelAI.Get());
		railBuilder.pathBuildParams = {
		...
		if(additionalHgStation.stationGroup == null) {
			HgLog.Warning("TrainRoute: additionalHgStation was removed."); // ?????????????????????????????
			Rollback();
			return 1;
		}
		...
		if(route.GetLastRoute().returnRoute != null) {
			route.GetLastRoute().RemoveReturnRoute(); // dest???return route????????????????
		}
	...
	function DoBuild() {
		...
		local railBuilderReturnDest;
		{
			...
			if(!returnDestStation.BuildExec()) {
				Place.AddNgPlace(destPlace,route.cargo,AIVehicle.VT_RAIL);
				HgLog.Warning("TrainRoute: cannot build returnDestStation");
				Rollback(); // TODO: ??transfer station????????????route????????(???????double depot???????)
				return null;
			}
			...
			railBuilderReturnDest.pathArrivalGetter = GetterFunction( function():(route, railBuilderReturnDest) {
					...
					return pathForReturnDest.SubPathStart(railBuilderReturnDest.buildedPath1.path.GetFirstTile()); // TODO: railBuilderReturnDestDeparture??????????????
				});
			...
			railBuilderReturnDest.eventPoller = SquirrelAI.Get();
		...
		local transferStation = SquirrelAI.Get().GetBuildableStationByPath(
			returnPath,
			10,
		...
		local railBuilderPathToTransfer;
		{
			...
			}), transferStation, 80, SquirrelAI.Get(), null, false);
			...
			AddRollback(railBuilderTransferToPath.buildedPath); // TODO Rollback??????????????????limit date:300????????
			/*
			...
			}), transferStation, 80, SquirrelAI.Get());
...

		{
			...
			route.slopesTable.clear(); // TODO: ChangeDestination????????????????
...
class MainLineRefactor extends RouteModificatin {
	...
	function RefactorMainLine() {
		...
		local gapLength = 5;
		local removeLength = SquirrelAI.Get().IsEnableVehicleBreakdowns() ? 10 : 20; // double depot????????
		...
		local railBuilder = TwoWayPtoPRailBuilder(departures[0], arrivals[0], departures[1], arrivals[1], SquirrelAI.Get().pathFindLimit, SquirrelAI.Get());
		railBuilder.pathBuildParams = {
			engine = route.GetLatestEngineSet().engine
			cargo = route.cargo
			platformLength = route.GetPlatformLength()
		...
		//railBuilder.notFlatten = true; // ???????flatten?????
		if(!railBuilder.Build()) {

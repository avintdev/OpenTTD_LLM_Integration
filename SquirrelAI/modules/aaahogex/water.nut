...
class WaterRoute extends CommonRoute {
	...
	function GetMaxTotalVehicles() {
		return SquirrelAI.Get().maxShips;
	}
	...
	function SetPath(path) {
		...
		local count = 0;
		local useBouy = SquirrelAI.Get().openttdVersion < 14;
		local prev = null;
		while(path != null) {
	...
	function OnVehicleLost(vehicle) {
		...
		if(CommonRoute.vehicleRemoving.rawin(vehicle) && (AIOrder.OF_STOP_IN_DEPOT & AIOrder.GetOrderFlags(vehicle, AIOrder.ORDER_CURRENT)) != 0) {
			...
			local o1 = AIVehicle.SendVehicleToDepot(vehicle); // ??depot?????
			local o2 = AIVehicle.SendVehicleToDepot(vehicle); // ?????
			HgLog.Warning("ET_VEHICLE_LOST: SendVehicleToDepot (retry) "+o1+" "+o2+" "+f+" "+HgTile(dest));
		} else {
			local f = AIOrder.GetOrderFlags(vehicle, AIOrder.ORDER_CURRENT);
			local dest = AIOrder.GetOrderDestination(vehicle, AIOrder.ORDER_CURRENT);
			local load = AIVehicle.GetCargoLoad(vehicle, cargo); // ????????vehicle?depot????????????????????????
...
/*	
	...
	static function CanBuild(from, to, cargo, isBidirectional) {
		//local usableTrain = !TrainRoute.IsTooManyVehiclesForNewRoute(TrainRoute) && !SquirrelAI.Get().roiBase;
		local key = from.GetLocation() + "-" + to.GetLocation() +"-" + cargo +"-" + SquirrelAI.Get().roiBase; // +"-" + usableTrain;
	...
	static function _CanBuild(from, to, cargo, isBidirectional) {
		local usableTrain = !isBidirectional && !TrainRoute.IsTooManyVehiclesForNewRoute(TrainRoute); // && !SquirrelAI.Get().roiBase;
...
class WaterRouteBuilder extends CommonRouteBuilder {
	...
	constructor(dest, src, cargo, options={}) {
		...
		retryIfNoPathUsingSharableStation = false;// ??????????? true;
	...
	function BuildCompoundRoute(isDestFarFromSea, coastTile) {
		...
		if(isDestFarFromSea) {
			...
			local srcRoute = WaterRouteBuilder( coastPlace, src, cargo, {
				...
				notUseCompoundRoute = true,  // Compound?????????
				notNeedToMeetDemand = true } ).Build();
			if(srcRoute == null) {
				return null;
			}
			srcRoute.isBuilding = true; //????????????????????????Remove()???
			...
			destRoute.isBuilding = true; //????????????????????????Remove()???
...
class CanalStationFactory extends StationFactory {
	...
	function GetPlatformLength() {
		return 1; // CeveredArea???????CanalStation?platformTile??????????1??????(??station?platformTile?????)
	}
...
class WaterStation extends HgStation {
	...
	function Build(levelTiles=false,isTestMode=true) {
		...
		if(!water) {
			...
			if(BuildPlatform(isTestMode,true)) {
				...
				/*if(!WaterRoute.usedTiles.rawin(HgTile.DIR4Index[GetDir4Index()])) { ????????dock????????
					return false;
				}*/
				return true;
			}
		...
		//??????????????AICompany.GetBankBalance(AICompany.COMPANY_SELF) > GetInflatedMoney(2000000);
		local hgTile = HgTile(platformTile);
		if(!SquirrelAI.Get().waterRemovable) {
			if(water) {
				if(!isTestMode) {
	...
	function _Build(dir, isTestMode=true,) {
		...
		if(SquirrelAI.Get().waterRemovable) {
			if(!AITile.IsWaterTile(next) && (SquirrelAI.Get().roiBase || !AITile.IsCoastTile(next))) {
				if(!isTestMode) {
	...
	function GetBuildableScore() {
		...
		if(place != null && place instanceof TownCargo 
				&& (!SquirrelAI.Get().IsDistantJoinStations() || SquirrelAI.Get().IsAvoidExtendCoverageAreaInTowns())
...
class CanalStation extends HgStation {
	...
	function Build(levelTiles=true,isTestMode=true) {
		local waterRemovable = SquirrelAI.Get().waterRemovable;
	...
	function BuildDepotX(x) {
		local canals = [At(0,2),At(0,3),At(x,1),At(x,2),At(x,3)];// (0,2)?Lock?????????????
		{
...
class WaterPathBuilder {
	...
	function BuildPath(starts /*dst*/, goals /*src*/, suppressInterval=false) {
		...
		local pathfinder = WaterPathFinder();
		local pathFindLimit = max(SquirrelAI.Get().roiBase ? 15 : 15, distance / 10);
...
class WaterPathFinder {
	...
	static function BuildLock(prev,next,prevLevel,execMode=false) {
		...
		if(AITile.IsStationTile(next2) || AITile.IsStationTile(prev2)) { // ?????????depot??????????????
			return null;
		}
		local newLevel = null;
	...
	function _Neighbours(self, path, cur_tile) {
		...
		foreach (offset in haveLock ? [(cur_tile - prev)/2] : HgTile.DIR4Index) {
			...
			local dir = 0xff; //normalMode ? 0xff : self.GetDirection(next_tile,cur_tile); ????????????????????
			if (AIMarine.AreWaterTilesConnected(cur_tile,next_tile)) {
				...
				if(AIMarine.BuildCanal(next_tile)
						&& !AIMarine.IsLockTile(cur_tile) /*Lock/Depot????Canal???????????(TODO:???????????)*/
				...
				if(!SquirrelAI.Get().roiBase && AITile.GetMaxHeight(next_tile)==1 && WaterPathFinder.LowerToZero(next_tile)) {
...
//		local end = AIMap.GetTileIndex(1,y);
		...
		while(cur >= end) {
			if(tileCoastId.rawin(cur) && tileCoastId[cur] == id) {
				...
				if(myCoast) { // ????????coast
					if(AITile.IsSeaTile(prev) != AITile.IsSeaTile(cur)) { //TODO: ?????????
						boundCount ++;
					}
					myCoast = false;
				}
